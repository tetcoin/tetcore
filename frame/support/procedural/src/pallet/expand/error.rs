...
pub fn expand_error(def: &mut Def) -> proc_macro2::TokenStream {
	...
	let phantom_variant: syn::Variant = syn::parse_quote!(
		#[doc(hidden)]
		__Ignore(
			#frame_support::tetcore_std::marker::PhantomData<(#type_use_gen)>,
			#frame_support::Never,
		)
	);
	...
	quote::quote_spanned!(error.attr_span =>
		impl<#type_impl_gen> #frame_support::tetcore_std::fmt::Debug for #error_ident<#type_use_gen>
			#config_where_clause
		{
			fn fmt(&self, f: &mut #frame_support::tetcore_std::fmt::Formatter<'_>)
				-> #frame_support::tetcore_std::fmt::Result
			{
