...
impl SlashingSpans {
	...
	pub(crate) fn iter(&'_ self) -> impl Iterator<Item = SlashingSpan> + '_ {
		...
		tetcore_std::iter::once(last).chain(prior)
	}
	...
	fn prune(&mut self, window_start: EraIndex) -> Option<(SpanIndex, SpanIndex)> {
		...
		self.last_start = tetcore_std::cmp::max(self.last_start, window_start);
		pruned
	}
}
...
struct InspectingSpans<'a, T: Config + 'a> {
	...
	_marker: tetcore_std::marker::PhantomData<T>,
}
...
fn fetch_spans<'a, T: Config + 'a>(
	...
	InspectingSpans {
		...
		paid_out,
		reward_proportion,
		_marker: tetcore_std::marker::PhantomData,
	}
}
...
impl<'a, T: 'a + Config> InspectingSpans<'a, T> {
	...
	fn add_slash(&mut self, amount: BalanceOf<T>, slash_era: EraIndex) {
		*self.slash_of += amount;
		self.spans.last_nonzero_slash = tetcore_std::cmp::max(self.spans.last_nonzero_slash, slash_era);
	}
