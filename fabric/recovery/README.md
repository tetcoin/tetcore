...
- [`recovery::Trait`](https://docs.rs/noble-recovery/latest/noble_recovery/trait.Trait.html)
- [`Call`](https://docs.rs/noble-recovery/latest/noble_recovery/enum.Call.html)

## Overview

The Recovery noble is an M-of-N social recovery tool for users to gain
access to their accounts if the private key or other authentication mechanism
is lost. Through this noble, a user is able to make calls on-behalf-of another
...
8. Then the account owner should then call `remove_recovery` to remove the recovery
   ...
   deposit they placed.
9. Using `as_recovered`, the account owner is able to call any other nobles
...
this noble makes it near-zero cost to re-configure the recovery settings and
remove/replace friends who are acting inappropriately.

### Safety Considerations

It is important to note that this is a powerful noble that can compromise the
