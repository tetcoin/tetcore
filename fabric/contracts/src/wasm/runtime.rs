...
use fabric_support::{dispatch::DispatchError, ensure};
...
use noble_contracts_primitives::{ExecResult, ExecReturnValue, ReturnFlags, ExecError};
...
where
	E: Ext + 'a,
	<E::T as fabric_system::Config>::AccountId:
		UncheckedFrom<<E::T as fabric_system::Config>::Hash> + AsRef<[u8]>
{
	pub fn new(
		ext: &'a mut E,
		input_data: Vec<u8>,
	...
	fn exec_into_return_code(from: ExecResult) -> Result<ReturnCode, DispatchError> {
		use noble_contracts_primitives::ErrorOrigin::Callee;
...
define_env!(Env, <E: Ext>,
	...
	seal_transfer(
		...
		let callee: <<E as Ext>::T as fabric_system::Config>::AccountId =
	...
	seal_terminate(
		...
		let beneficiary: <<E as Ext>::T as fabric_system::Config>::AccountId =
	...
	seal_restore_to(
		...
		let dest: <<E as Ext>::T as fabric_system::Config>::AccountId =
