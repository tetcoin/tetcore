...
//! Benchmarks for the contracts noble
...
use fabric_system::{Module as System, RawOrigin};
...
benchmarks! {
	...
	seal_instantiate {
		...
		let value_bytes = value.encode();
		let value_len = value_bytes.len();
		let addr_len = tetcore_std::mem::size_of::<T::AccountId>();
	...
	//     -p noble_contracts -e print_schedule --no-median-slopes --no-min-squares
	#[extra]
	print_schedule {
		#[cfg(feature = "std")]
		{
