...
use noble_contracts_primitives::{ErrorOrigin, ExecError, ExecReturnValue, ExecResult, ReturnFlags};

pub type AccountIdOf<T> = <T as fabric_system::Config>::AccountId;
pub type MomentOf<T> = <<T as Config>::Time as Time>::Moment;
pub type SeedOf<T> = <T as fabric_system::Config>::Hash;
pub type BlockNumberOf<T> = <T as fabric_system::Config>::BlockNumber;
pub type StorageKey = [u8; 32];

/// A type that represents a topic of an event. At the moment a hash is used.
pub type TopicOf<T> = <T as fabric_system::Config>::Hash;
	...
	V: Vm<T, Executable = E>,
{
	...
	pub fn top_level(origin: T::AccountId, cfg: &'a ConfigCache<T>, vm: &'a V, loader: &'a L) -> Self {
		ExecutionContext {
			...
			block_number: <fabric_system::Module<T>>::block_number(),
		}
	}
	...
	pub fn instantiate(
		...
		let output = fabric_support::storage::with_transaction(|| {
			...
			use fabric_support::storage::TransactionOutcome::*;
			match output {
				Ok(_) => Commit(output),
		...
		where F: FnOnce(&mut ExecutionContext<T, V, L>) -> ExecResult
	{
		...
		fabric_support::storage::with_transaction(|| {
...
fn deposit_event<T: Config>(
	...
	<fabric_system::Module<T>>::deposit_event_indexed(
		&*topics,
...
mod tests {
	...
	fn events() -> Vec<Event<Test>> {
		<fabric_system::Module<Test>>::events()
			.into_iter()
	...
	impl<'a> MockLoader<'a> {
		...
		fn insert(&mut self, f: impl Fn(MockCtx) -> ExecResult + 'a) -> CodeHash<Test> {
			...
			let code_hash = <Test as fabric_system::Config>::Hash::from_low_u64_be(self.counter);
