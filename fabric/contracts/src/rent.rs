...
use noble_contracts_primitives::{ContractAccessError, RentProjection, RentProjectionResult};
use tp_runtime::{
	DispatchError,
	...
	T::AccountId: UncheckedFrom<T::Hash> + AsRef<[u8]>
{
	...
	pub fn charge(account: &T::AccountId) -> Result<Option<ContractInfo<T>>, DispatchError> {
		...
		let current_block_number = <fabric_system::Module<T>>::block_number();
	...
	pub fn restore_to(
		...
		let current_block = <fabric_system::Module<T>>::block_number();
