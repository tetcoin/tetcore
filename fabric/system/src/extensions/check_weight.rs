...
use fabric_support::{
	traits::{Get},
	weights::{PostDispatchInfo, DispatchInfo, DispatchClass, priority::FabricTransactionPriority},
};
	...
	fn get_priority(info: &DispatchInfoOf<T::Call>) -> TransactionPriority {
		match info.class {
			...
			DispatchClass::Normal =>
				FabricTransactionPriority::Normal(info.weight.into()).into(),
			// Don't use up the whole priority space, to allow things like `tip` to be taken into
			// account as well.
			DispatchClass::Operational =>
				FabricTransactionPriority::Operational(info.weight.into()).into(),
...
mod tests {
	...
	use fabric_support::{assert_ok, assert_noop};
	use fabric_support::weights::{Weight, Pays};
	...
	fn signed_ext_check_weight_works() {
		new_test_ext().execute_with(|| {
			...
			let priority = CheckWeight::<Test>(PhantomData)
				...
				.unwrap()
				.priority;
			assert_eq!(priority, fabric_support::weights::priority::LIMIT + 100);
		})
	}
