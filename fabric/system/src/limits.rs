...
//!
//! FABRIC defines two resources that are limited within a block:
//! - Weight (execution cost/time)
//! - Length (block size)
//!
//! `fabric_system` tracks consumption of each of these resources separately for each
//! `DispatchClass`. This module contains configuration object for both resources,
//! which should be passed to `fabric_system` configuration when runtime is being set up.

use fabric_support::weights::{Weight, DispatchClass, constants, PerDispatchClass, OneOrMany};
...
/// `on_initialize` noble callbacks are invoked and their cost is added before any extrinsic
