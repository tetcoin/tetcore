...
//! # System Noble
//!
//! The System noble provides low-level access to core types and cross-cutting utilities.
...
//! ## Overview
//!
//! The System noble defines the core data types used in a Tetcore runtime.
//! It also provides several utility functions (see [`Noble`]) for other FABRIC nobles.
//!
...
//!
//! The System noble does not implement any dispatchable functions.
//!
//! ### Public Functions
//!
//! See the [`Noble`] struct for details of publicly available functions.
//!
//! ### Signed Extensions
//!
//! The System noble defines the following extensions:
//!
...
pub mod noble {
	...
	pub trait Config: 'static + Eq + Clone {
		...
		///
		/// Expects the `NobleInfo` type that is being generated by `construct_runtime!` in the
		/// runtime.
		///
	...
	impl<T: Config> Noble<T> {
		...
		// that's not possible at present (since it's within the noble macro).
...
pub enum RawOrigin<AccountId> {
	...
	/// * or unsigned transaction validated by a noble.
	None,
}
...
impl<T: Config> Module<T> {
	...
	/// Inform the system noble of some additional weight that should be accounted for, in the
	/// current block.
	///
	/// NOTE: use with extra care; this function is made public only be used for certain nobles
	...
	/// Get the basic externalities for this noble, useful for tests.
	#[cfg(any(feature = "std", test))]
	...
	/// (e.g., called `on_initialize` for all nobles).
