...
//! ## Usage
//!
//! Please refer to [`example-offchain-worker`](../../noble_example_offchain_worker/index.html) for
...
//! See [`PricePayload`](../../noble_example_offchain_worker/struct.PricePayload.html)
//!
...
use fabric_support::{debug, RuntimeDebug};
...
pub struct SubmitTransaction<T: SendTransactionTypes<OverarchingCall>, OverarchingCall> {
	_phantom: tetcore_std::marker::PhantomData<(T, OverarchingCall)>
}
...
pub struct Signer<T: SigningTypes, C: AppCrypto<T::Public, T::Signature>, X = ForAny> {
	...
	_phantom: tetcore_std::marker::PhantomData<(X, C)>,
}
...
pub trait SendTransactionTypes<LocalCall> {
	...
	///
	/// This has additional bound to be able to be created from noble-local `Call` types.
...
/// fees, or mortality depending on the `noble` being called).
