...
//!
//! This noble is currently not maintained and should not be used in production until further
//! notice.
//!
//! ---
//!
...
decl_event!(
	pub enum Event<T> where <T as fabric_system::Config>::AccountId {
		/// Reaped \[voter, reaper\].
...
impl<T: Config> Module<T> {
		...
		if desired_seats == 0 {
			...
			let (next_possible, count, coming) =
				if let Some((tally_end, comers, leavers)) = Self::next_finalize() {
					...
					(<fabric_system::Module<T>>::block_number(), c.len(), 0)
				};
	...
	fn start_tally() {
		...
		let number = <fabric_system::Module<T>>::block_number();
		let expiring =
	...
	fn finalize_tally() -> DispatchResult {
		...
		let leaderboard: Vec<(BalanceOf<T>, T::AccountId)> = <Leaderboard<T>>::take()
			.unwrap_or_default();
		let new_expiry = <fabric_system::Module<T>>::block_number() + Self::term_duration();
