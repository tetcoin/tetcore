...
use fabric_system::{ensure_none, ensure_root, ensure_signed};
use tp_runtime::{
	generic::DigestItem,
	traits::Zero,
...
pub trait Config: fabric_system::Config {
	...
	/// `()`) you must use this noble's `ValidateUnsigned` in the runtime
	/// definition.
...
impl<T: Config> Module<T> {
	...
	pub fn schedule_pause(in_blocks: T::BlockNumber) -> DispatchResult {
		if let StoredState::Live = <State<T>>::get() {
			let scheduled_at = <fabric_system::Module<T>>::block_number();
