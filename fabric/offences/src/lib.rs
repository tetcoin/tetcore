...
type ReportIdOf<T> = <T as fabric_system::Config>::Hash;

/// Type of data stored as a deferred offence
pub type DeferredOffenceOf<T> = (
	Vec<OffenceDetails<<T as fabric_system::Config>::AccountId, <T as Config>::IdentificationTuple>>,
	Vec<Perbill>,
	SessionIndex,
);
				...
				deferred.retain(|(offences, perbill, session)| {
					if consumed >= limit {
						...
						match T::OnOffenceHandler::on_offence(&offences, &perbill, *session) {
							...
							Err(_) => {
								debug::native::error!(
									target: "noble-offences",
									"re-submitting a deferred slash returned Err at {}. This should not happen with noble-staking",
									now,
								);
								true
							},
