...
use fabric_support::{decl_module, decl_event, decl_storage, Parameter, transactional};
use fabric_support::{
...
pub trait Config: fabric_system::Config {
	...
	type Call: Parameter + Dispatchable<Origin=Self::Origin, PostInfo=PostDispatchInfo>
		+ GetDispatchInfo + From<fabric_system::Call<Self>>
...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		/// bypassing `fabric_system::Config::BaseCallFilter`).
		///
		/// # <weight>
		...
		fn as_derivative(origin, index: u16, call: Box<<T as Config>::Call>) -> DispatchResultWithPostInfo {
			...
			origin.set_caller_from(fabric_system::RawOrigin::Signed(pseudonym));
			let info = call.get_dispatch_info();
