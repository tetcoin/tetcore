...
pub mod example {
	use super::*;
	use fabric_support::dispatch::WithPostDispatchInfo;
...
parameter_types! {
	...
	pub BlockWeights: fabric_system::limits::BlockWeights =
		fabric_system::limits::BlockWeights::simple_max(Weight::max_value());
}
...
impl Filter<Call> for TestBaseCallFilter {
	fn filter(c: &Call) -> bool {
		match *c {
			...
			Call::Balances(noble_balances::Call::transfer(..)) => true,
			Call::Utility(_) => true,
			// For benchmarking, this acts as a noop call
			Call::System(fabric_system::Call::remark(..)) => true,
			// For tests
			Call::Example(_) => true,
			_ => false,
		}
...
use fabric_system::Call as SystemCall;
...
fn last_event() -> Event {
	fabric_system::Module::<Test>::events().pop().map(|e| e.event).expect("Event expected")
}
...
fn as_derivative_filters() {
	new_test_ext().execute_with(|| {
		assert_err_ignore_postinfo!(Utility::as_derivative(
			Origin::signed(1),
			1,
			Box::new(Call::Balances(noble_balances::Call::transfer_keep_alive(2, 1))),
...
fn batch_with_root_works() {
	new_test_ext().execute_with(|| {
		let k = b"a".to_vec();
		let call = Call::System(fabric_system::Call::set_storage(vec![(k.clone(), k.clone())]));
...
fn batch_with_signed_filters() {
	new_test_ext().execute_with(|| {
		assert_ok!(
			Utility::batch(Origin::signed(1), vec![
				Call::Balances(noble_balances::Call::transfer_keep_alive(2, 1))
			]),
		);
	...
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			...
			DispatchErrorWithPostInfo {
				...
				error: noble_balances::Error::<Test, _>::InsufficientBalance.into()
			}
		);
		assert_eq!(Balances::free_balance(1), 10);
		assert_eq!(Balances::free_balance(2), 10);
