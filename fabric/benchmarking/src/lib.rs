...
macro_rules! benchmarks_iter {
	...
	(
		...
		$crate::benchmarks_iter! {
			...
			$name { $( $code )* }: {
				<
					Call<T $(, $instance)? > as $crate::fabric_support::traits::UnfilteredDispatchable
...
macro_rules! selected_benchmark {
	(
		...
		// The list of available benchmarks for this noble.
		#[allow(non_camel_case_types)]
...
macro_rules! impl_benchmark {
	(
		...
		impl<T: Config $(<$instance>, I: Instance)? >
			...
			where T: fabric_system::Config, $( $where_clause )*
		{
			...
			fn run_benchmark(
				...
				let extrinsic = tetcore_std::str::from_utf8(extrinsic)
				...
				let whitelisted_caller_key =
					<fabric_system::Account::<T> as fabric_support::storage::StorageMap<_,_>>::hashed_key_for(
				...
				| -> Result<(), &'static str> {
					...
					for _ in 0..repeat {
						...
						if $crate::Zero::is_zero(&fabric_system::Module::<T>::block_number()) {
							fabric_system::Module::<T>::set_block_number(1u32.into());
						}
...
pub fn show_benchmark_debug_info(
	instance_string: &[u8],
	benchmark: &[u8],
	lowest_range_values: &tetcore_std::prelude::Vec<u32>,
	highest_range_values: &tetcore_std::prelude::Vec<u32>,
	steps: &tetcore_std::prelude::Vec<u32>,
	repeat: &u32,
	verify: &bool,
	error_message: &str,
	...
	tp_runtime::format_runtime_string!(
		...
		* Verify: {:?}\n\
		* Error message: {}",
		tetcore_std::str::from_utf8(instance_string)
		.expect("it's all just strings ran through the wasm interface. qed"),
		tetcore_std::str::from_utf8(benchmark)
		.expect("it's all just strings ran through the wasm interface. qed"),
		lowest_range_values,
		highest_range_values,
		steps,
...
/// This macro adds noble benchmarks to a `Vec<BenchmarkBatch>` object.
///
...
/// ````
///
/// Then add the nobles you want to benchmark to this object, using their crate name and generated
/// module struct:
///
/// ```ignore
...
///
/// In the case where you have multiple instances of a noble that you need to separately benchmark,
/// the name of your module struct will be used as a suffix to your outputted weight file. For
/// example:
///
/// ```ignore
/// add_benchmark!(params, batches, noble_balances, Balances); // noble_balances.rs
/// add_benchmark!(params, batches, noble_collective, Council); // noble_collective_council.rs
/// add_benchmark!(params, batches, noble_collective, TechnicalCommittee); // noble_collective_technical_committee.rs
/// ```
///
...
/// add_benchmark!(params, batches, noble_collective, Council2); // noble_collective_council_2.rs
/// ```
...
macro_rules! add_benchmark {
	( $params:ident, $batches:ident, $name:path, $( $location:tt )* ) => (
		...
		if &noble[..] == &name_string[..] || &noble[..] == &b"*"[..] {
			if &noble[..] == &b"*"[..] || &benchmark[..] == &b"*"[..] {
				for benchmark in $( $location )*::benchmarks(*extra).into_iter() {
					$batches.push($crate::BenchmarkBatch {
						noble: name_string.to_vec(),
