...
//! # Bounties Module ( noble-bounties )
//!
//! ## Bounty
//!
...
use fabric_support::{decl_module, decl_storage, decl_event, ensure, decl_error};
...
use fabric_support::dispatch::DispatchResultWithPostInfo;
...
type BalanceOf<T> = noble_treasury::BalanceOf<T>;

type PositiveImbalanceOf<T> = noble_treasury::PositiveImbalanceOf<T>;
...
// noble-bounties uses Treasury for storage.
...
impl<T: Config> noble_treasury::SpendFunds<T> for Module<T> {
	fn spend_funds(
