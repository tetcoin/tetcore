...
impl noble_treasury::Config for Test {
	...
	type ApproveOrigin = fabric_system::EnsureRoot<u128>;
	type RejectOrigin = fabric_system::EnsureRoot<u128>;
	type Event = Event;
	type OnSlash = ();
...
pub fn new_test_ext() -> tet_io::TestExternalities {
	...
	noble_treasury::GenesisConfig::default().assimilate_storage::<Test, _>(&mut t).unwrap();
	t.into()
}
...
fn close_bounty_works() {
	new_test_ext().execute_with(|| {
		...
		assert!(!noble_treasury::Proposals::<Test>::contains_key(0));
...
fn assign_curator_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(Bounties::accept_curator(Origin::signed(4), 0), noble_balances::Error::<Test, _>::InsufficientBalance);
