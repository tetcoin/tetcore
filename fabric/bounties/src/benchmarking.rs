...
benchmarks! {
	...
	unassign_curator {
		...
		Bounties::<T>::on_initialize(T::BlockNumber::zero());
		let bounty_id = BountyCount::get() - 1;
		fabric_system::Module::<T>::set_block_number(T::BountyUpdatePeriod::get() + 1u32.into());
	...
	claim_bounty {
		...
		fabric_system::Module::<T>::set_block_number(T::BountyDepositPayoutDelay::get());
	...
	spend_funds {
		...
		<Bounties<T> as noble_treasury::SpendFunds<T>>::spend_funds(
