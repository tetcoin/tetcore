...
use fabric_support::dispatch::DispatchError;
...
pub trait Config: fabric_system::Config {
	...
	type Call: Parameter + Dispatchable<Origin=Self::Origin, PostInfo=PostDispatchInfo>
		+ GetDispatchInfo + From<fabric_system::Call<Self>> + IsSubType<Call<Self>>
		+ IsType<<Self as fabric_system::Config>::Call>;
...
impl<T: Config> Module<T> {
	...
	fn do_proxy(
		...
		let mut origin: T::Origin = fabric_system::RawOrigin::Signed(real).into();
		origin.add_filter(move |c: &<T as fabric_system::Config>::Call| {
			let c = <T as Config>::Call::from_ref(c);
			// We make sure the proxy call does access this noble to change modify proxies.
			match c.is_sub_type() {
