...
impl_outer_dispatch! {
	pub enum Call for Test where origin: Origin {
		fabric_system::System,
		noble_balances::Balances,
		proxy::Proxy,
		noble_utility::Utility,
	}
}
...
impl noble_utility::Config for Test {
	type Event = TestEvent;
	type Call = Call;
	type WeightInfo = ();
}
...
impl InstanceFilter<Call> for ProxyType {
	fn filter(&self, c: &Call) -> bool {
		match self {
			ProxyType::Any => true,
			ProxyType::JustTransfer => matches!(c, Call::Balances(noble_balances::Call::transfer(..))),
...
type Utility = noble_utility::Module<Test>;
...
use noble_balances::Event as BalancesEvent;
use noble_utility::Call as UtilityCall;
use noble_utility::Event as UtilityEvent;
