...
//!
//! - \[`<INSERT_CUSTOM_NOBLE_NAME>::Config`](./trait.Config.html)
//! - \[`Call`](./enum.Call.html)
...
struct WeightForSetDummy<T: noble_balances::Config>(BalanceOf<T>);

impl<T: noble_balances::Config> WeighData<(&BalanceOf<T>,)> for WeightForSetDummy<T>
{
...
impl<T: noble_balances::Config> ClassifyDispatch<(&BalanceOf<T>,)> for WeightForSetDummy<T> {
...
impl<T: noble_balances::Config> PaysFee<(&BalanceOf<T>,)> for WeightForSetDummy<T> {
	fn pays_fee(&self, _target: (&BalanceOf<T>,)) -> Pays {
		Pays::Yes
	}
}

/// A type alias for the balance type from this noble's point of view.
type BalanceOf<T> = <T as noble_balances::Config>::Balance;

/// Our noble's configuration trait. All our types and constants go in here. If the
/// noble is dependent on specific other nobles, then their configuration traits
/// should be added to our implied traits list.
///
/// `fabric_system::Config` should always be included in our implied traits.
pub trait Config: noble_balances::Config {
	/// The overarching event type.
	type Event: From<Event<Self>> + Into<<Self as fabric_system::Config>::Event>;
}

decl_storage! {
	// A macro for the Storage trait, and its implementation, for this noble.
	// This allows for type-safe usage of the Tetcore storage database, so you can
	// keep things around between blocks.
	//
	// It is important to update your storage name so that your noble's
	// storage items are isolated from other nobles.
	// ---------------------------------vvvvvvv
	trait Store for Module<T: Config> as Example {
		...
		// `fabric_support::StorageValue`. For map items, you'll get a type which
		// implements `fabric_support::StorageMap`.
		//
		// If they have a getter (`get(getter_name)`), then your noble will come
...
decl_event!(
	...
	pub enum Event<T> where B = <T as noble_balances::Config>::Balance {
...
//
// When you come to `impl` them later in the noble, you must specify the full type for `origin`:
//
// `fn foo(origin: T::Origin, bar: Bar, baz: Baz) { ... }`
//
// There are three entries in the `fabric_system::Origin` enum that correspond
...
decl_module! {
	...
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		/// Deposit one of this noble's events by using the default implementation.
		...
		/// This is just a simple example of how to interact with the noble from the external
		/// world.
		...
		// some value from the sender's account (`Balances` Noble has a `reserve` function for
		...
		// information about its dispatch. FABRIC System and FABRIC Executive noble then use this
...
// The main implementation block for the noble. Functions here fall into three broad
// categories:
// - Public interface. These are functions that are `pub` and generally fall into inspector
// functions that do not write to storage and operation functions that do.
// - Private functions. These are your usual private utilities unavailable to other nobles.
...
// Similar to other FABRIC nobles, your noble can also define a signed extension and perform some
...
// bounds. As a convention, you can follow this approach to create an extension for your noble:
...
// method. This example will not cover this type of extension. See `CheckRuntime` in FABRIC System
// for an example.
//
...
// variant is given to each function of `SignedExtension`. Hence, you can filter based on noble or
...
mod tests {
	...
	// For testing the noble, we construct most of a mock runtime. This means
	// first constructing a configuration type (`Test`) which `impl`s each of the
	// configuration traits of nobles we want to use.
