...
//!
//! The [Staking noble](../noble_staking/index.html) uses the Session noble to get the validator
//! set.
//!
//! ```
...
//! ## Related Modules
//!
//! - [Staking](../noble_staking/index.html)
...
pub trait SessionManager<ValidatorId> {
	...
	/// End the session.
	///
	/// Because the session noble can queue validator set the ending session can be lower than the
...
pub trait Config: fabric_system::Config {
	...
	/// which in combination with `noble_staking` forces a new era.
	...
	add_extra_genesis {
		...
		build(|config: &GenesisConfig<T>| {
			...
			for (account, val, keys) in config.keys.iter().cloned() {
				...
				assert!(fabric_system::Module::<T>::inc_consumers(&account).is_ok());
			}

			let initial_validators_0 = T::SessionManager::new_session(0)
				.unwrap_or_else(|| {
					fabric_support::print("No initial validator provided by `SessionManager`, use \
...
impl<T: Config> Module<T> {
	...
	fn do_set_keys(account: &T::AccountId, keys: T::Keys) -> dispatch::DispatchResult {
		...
		fabric_system::Module::<T>::inc_consumers(&account).map_err(|_| Error::<T>::NoAccount)?;
		let old_keys = Self::inner_set_keys(&who, keys)?;
		if old_keys.is_some() {
			let _ = fabric_system::Module::<T>::dec_consumers(&account);
	...
	fn do_purge_keys(account: &T::AccountId) -> DispatchResult {
		...
		fabric_system::Module::<T>::dec_consumers(&account);
