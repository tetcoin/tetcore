...
//! An opt-in utility for tracking historical sessions in FABRIC-session.
//!
...
use fabric_support::{decl_module, decl_storage};
use fabric_support::{Parameter, print};
...
impl<T: Config> Module<T> {
	...
	pub fn prune_up_to(up_to: SessionIndex) {
		<Self as Store>::StoredRange::mutate(|range| {
			...
			let up_to = tetcore_std::cmp::min(up_to, end);
...
pub struct NoteHistoricalRoot<T, I>(tetcore_std::marker::PhantomData<(T, I)>);
...
impl<T: Config, D: AsRef<[u8]>> fabric_support::traits::KeyOwnerProofSystem<(KeyTypeId, D)>
	for Module<T>
{
...
pub(crate) mod tests {
	...
	use fabric_support::traits::{KeyOwnerProofSystem, OnInitialize};
	use fabric_support::BasicExternalities;
	...
	pub(crate) fn new_test_ext() -> tet_io::TestExternalities {
		...
		BasicExternalities::execute_with_storage(&mut t, || {
			for (ref k, ..) in &keys {
				fabric_system::Module::<Test>::inc_providers(k);
			}
		});
