...
use fabric_support::{parameter_types, BasicExternalities};
...
use crate as noble_session;
#[cfg(feature = "historical")]
use crate::historical as noble_session_historical;
...
impl SessionHandler<u64> for TestSessionHandler {
	const KEY_TYPE_IDS: &'static [tp_runtime::KeyTypeId] = &[UintAuthorityId::ID];
...
pub fn new_test_ext() -> tet_io::TestExternalities {
	...
	BasicExternalities::execute_with_storage(&mut t, || {
		...
		fabric_system::Module::<Test>::inc_providers(&4);
		// An additional identity that we use.
		fabric_system::Module::<Test>::inc_providers(&69);
	});
	noble_session::GenesisConfig::<Test> { keys }.assimilate_storage(&mut t).unwrap();
