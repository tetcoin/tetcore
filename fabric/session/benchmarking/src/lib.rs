...
//! Benchmarks for the Session Noble.
...
use noble_session::{historical::Module as Historical, Module as Session, *};
use noble_staking::{
...
pub struct Module<T: Config>(noble_session::Module<T>);
pub trait Config: noble_session::Config + noble_session::historical::Config + noble_staking::Config {}

impl<T: Config> OnInitialize<T::BlockNumber> for Module<T> {
	fn on_initialize(n: T::BlockNumber) -> fabric_support::weights::Weight {
		noble_session::Module::<T>::on_initialize(n)
	}
}
...
benchmarks! {
	set_keys {
		...
		let v_controller = noble_staking::Module::<T>::bonded(&v_stash).ok_or("not stash")?;
		let keys = T::Keys::default();
		let proof: Vec<u8> = vec![0,1,2,3];
		// Whitelist controller account from further DB operations.
		let v_controller_key = fabric_system::Account::<T>::hashed_key_for(&v_controller);
		fabric_benchmarking::benchmarking::add_to_whitelist(v_controller_key.into());
...
fn check_membership_proof_setup<T: Config>(
	...
	noble_staking::ValidatorCount::put(n);
		...
		.enumerate()
	{
		...
		let controller = noble_staking::Module::<T>::bonded(validator).unwrap();
