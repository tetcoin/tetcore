...
use noble_balances::{BalanceLock, Error as BalancesError};
use fabric_system::{EnsureSignedBy, EnsureRoot};
...
impl Filter<Call> for BaseFilter {
	fn filter(call: &Call) -> bool {
		!matches!(call, &Call::Balances(noble_balances::Call::set_balance(..)))
	}
}
...
parameter_types! {
	...
	pub BlockWeights: fabric_system::limits::BlockWeights =
		fabric_system::limits::BlockWeights::simple_max(1_000_000);
}
...
impl noble_scheduler::Config for Test {
	type Event = Event;
	type Origin = Origin;
...
impl super::Config for Test {
	type Proposal = Call;
	type Event = Event;
	type Currency = noble_balances::Module<Self>;
...
type Scheduler = noble_scheduler::Module<Test>;
...
fn set_balance_proposal(value: u64) -> Vec<u8> {
	Call::Balances(noble_balances::Call::set_balance(42, value, 0)).encode()
}
