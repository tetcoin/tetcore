...
//! ## Overview
//!
//! The Democracy noble handles the administration of general stakeholder voting.
//!
...
//!
//! Every launch period - a length defined in the runtime - the Democracy noble
...
pub trait Config: fabric_system::Config + Sized {
	...
	type NoblesOrigin: From<system::RawOrigin<Self::AccountId>>;
...
decl_storage! {
	trait Store for Module<T: Config> as Democracy {
		// TODO: Refactor public proposal queue into its own noble.
		...
		// TODO: Refactor Preimages into its own noble.
		// https://github.com/tetcoin/tetcore/issues/5322
		pub Preimages:
	...
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		fn propose(origin,
			...
			if let Some((until, _)) = <Blacklist<T>>::get(proposal_hash) {
					...
					<fabric_system::Module<T>>::block_number() >= until,
		...
		fn veto_external(origin, proposal_hash: T::Hash) {
			...
			let until = <fabric_system::Module<T>>::block_number() + T::CooloffPeriod::get();
...
impl<T: Config> Module<T> {
	...
	pub fn internal_start_referendum(
		...
		<Module<T>>::inject_referendum(
			<fabric_system::Module<T>>::block_number() + T::VotingPeriod::get(),
			proposal_hash,
			threshold,
			delay
		)
	...
	fn try_delegate(
		...
		let votes = VotingOf::<T>::try_mutate(&who, |voting| -> Result<u32, DispatchError> {
			...
			tetcore_std::mem::swap(&mut old, voting);
			match old {
	...
	fn do_enact_proposal(proposal_hash: T::Hash, index: ReferendumIndex) -> DispatchResult {
		...
		if let Some(PreimageStatus::Available { data, provider, deposit, .. }) = preimage {
			if let Ok(proposal) = T::Proposal::decode(&mut &data[..]) {
				...
				let ok = proposal.dispatch(fabric_system::RawOrigin::Root.into()).is_ok();
		...
		if approved {
			...
			if status.delay.is_zero() {
				...
				if T::Scheduler::schedule_named(
					...
					fabric_support::print("LOGIC ERROR: bake_referendum/schedule_named failed");
				}
			}
		} else {
