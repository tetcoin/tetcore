...
//! Authorship tracking for FABRIC runtimes.
//!
...
use fabric_support::{decl_module, decl_storage, decl_error, dispatch, ensure};
use fabric_support::traits::{FindAuthor, VerifySeal, Get};
...
impl<T: Config> Module<T> {
	...
	pub fn author() -> T::AccountId {
		...
		let digest = <fabric_system::Module<T>>::digest();
	...
	) -> Result<Option<T::AccountId>, dispatch::DispatchError>
	{
...

		{
			...
			let parent_hash = <fabric_system::Module<T>>::block_hash(&parent_number);
		...
		let in_chain = <fabric_system::Module<T>>::block_hash(uncle.number()) == hash;
...
#[cfg(test)]
mod tests {
	use crate as noble_authorship;
	use super::*;
	use tet_core::H256;
	use tp_runtime::{
		traits::{BlakeTwo256, IdentityLookup}, testing::Header, generic::DigestItem,
	};
	use fabric_support::{parameter_types, ConsensusEngineId};
