...
pub trait Config: fabric_system::Config + noble_session::Config {}
...
#[cfg(test)]
mod tests {
	use crate as noble_authority_discovery;
	use super::*;
	...
	impl noble_session::SessionHandler<AuthorityId> for TestSessionHandler {
	...
	fn authorities_returns_current_and_next_authority_set() {
		...
		// `noble_session::OneSessionHandler::on_new_session`, thus its safe to use the same value
		// everywhere.
		...
		let second_authorities: Vec<AuthorityId> = vec![2, 3].into_iter()
			...
			.map(AuthorityId::from)
			.collect();
		// Needed for `noble_session::OneSessionHandler::on_new_session`.
		...
		noble_authority_discovery::GenesisConfig {
			keys: vec![],
		}
