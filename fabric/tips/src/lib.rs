...
use fabric_support::{decl_module, decl_storage, decl_event, ensure, decl_error, Parameter};
...
use fabric_support::traits::{Contains, ContainsLengthBound, OnUnbalanced, EnsureOrigin};
use codec::{Encode, Decode};
use fabric_system::{self as system, ensure_signed};
pub use weights::WeightInfo;

pub type BalanceOf<T> = noble_treasury::BalanceOf<T>;
pub type NegativeImbalanceOf<T> = noble_treasury::NegativeImbalanceOf<T>;

pub trait Config: fabric_system::Config + noble_treasury::Config {
...
// noble-tips uses Treasury for storage.
...
impl<T: Config> Module<T> {
	...
	fn payout_tip(hash: T::Hash, tip: OpenTip<T::AccountId, BalanceOf<T>, T::BlockNumber, T::Hash>) {
		...
		let max_payout = noble_treasury::Module::<T>::pot();
	...
	pub fn migrate_retract_tip_for_tip_new() {
		...
		use fabric_support::{Twox64Concat, migration::StorageKeyIterator};
