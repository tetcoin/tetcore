...
//! Scheduler noble benchmarking.

#![cfg(feature = "runtime-benchmarks")]

use super::*;
use tetcore_std::{vec, prelude::*};
use fabric_system::RawOrigin;
use fabric_support::{ensure, traits::OnInitialize};
...
fn fill_schedule<T: Config> (when: T::BlockNumber, n: u32) -> Result<(), &'static str> {
	...
	let call = fabric_system::Call::set_storage(vec![]);
	for i in 0..n {
		...
		Scheduler::<T>::do_schedule_named(
			...
			0,
			fabric_system::RawOrigin::Root.into(),
			call.clone().into(),
		)?;
	}
...
benchmarks! {
	schedule {
		...
		let priority = 0;
		// Essentially a no-op call.
		let call = Box::new(fabric_system::Call::set_storage(vec![]).into());
