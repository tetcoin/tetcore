...
//! for the origin: namely `fabric_system::Config::BaseCallFilter` for all origin
...
pub trait Config: system::Config {
	...
	type Origin: OriginTrait<NoblesOrigin =
		Self::NoblesOrigin> + From<Self::NoblesOrigin> + IsType<<Self as system::Config>::Origin>;

	/// The caller origin, overarching type of all nobles origins.
	type NoblesOrigin: From<system::RawOrigin<Self::AccountId>> + Codec + Clone + Eq;
...
#[cfg_attr(any(feature = "std", test), derive(PartialEq, Eq))]
#[derive(Clone, RuntimeDebug, Encode, Decode)]
pub struct ScheduledV2<Call, BlockNumber, NoblesOrigin, AccountId> {
...
pub type Scheduled<Call, BlockNumber, NoblesOrigin, AccountId> = ScheduledV2<Call, BlockNumber, NoblesOrigin, AccountId>;
...
decl_storage! {
	trait Store for Module<T: Config> as Scheduler {
		...
		pub Agenda: map hasher(twox_64_concat) T::BlockNumber
			=> Vec<Option<Scheduled<<T as Config>::Call, T::BlockNumber, T::NoblesOrigin, T::AccountId>>>;
			...
			queued.into_iter()
				...
				.scan(base_weight, |cumulative_weight, (order, (index, s))| {
					*cumulative_weight = cumulative_weight
					...
					let origin = <<T as Config>::Origin as From<T::NoblesOrigin>>::from(
						s.origin.clone()
					).into();
...
impl<T: Config> Module<T> {
	...
	/// Helper to migrate scheduler when the noble origin type has changed.
	pub fn migrate_origin<OldOrigin: Into<T::NoblesOrigin> + codec::Decode>() {
		Agenda::<T>::translate::<
	...
	fn do_cancel_named(origin: Option<T::NoblesOrigin>, id: Vec<u8>) -> DispatchResult {
...
impl<T: Config> schedule::Anon<T::BlockNumber, <T as Config>::Call, T::NoblesOrigin> for Module<T> {
...
impl<T: Config> schedule::Named<T::BlockNumber, <T as Config>::Call, T::NoblesOrigin> for Module<T> {
	type Address = TaskAddress<T::BlockNumber>;
...
mod tests {
	...
	use fabric_system::{EnsureOneOf, EnsureRoot, EnsureSignedBy};
	...
	mod logger {
		...
		decl_module! {
			...
			where
				...
				<T as system::Config>::Origin: OriginTrait<NoblesOrigin = OriginCaller>
			{
				fn deposit_event() = default;
	...
	parameter_types! {
		...
		pub BlockWeights: fabric_system::limits::BlockWeights =
			fabric_system::limits::BlockWeights::simple_max(2_000_000_000_000);
	}
	...
	fn basic_scheduling_works() {
		new_test_ext().execute_with(|| {
			...
			assert!(!<Test as fabric_system::Config>::BaseCallFilter::filter(&call));
	...
	fn on_initialize_weight_is_correct() {
		new_test_ext().execute_with(|| {
			let base_weight: Weight = <Test as fabric_system::Config>::DbWeight::get().reads_writes(1, 2);
			let base_multiplier = 0;
			let named_multiplier = <Test as fabric_system::Config>::DbWeight::get().writes(1);
			let periodic_multiplier = <Test as fabric_system::Config>::DbWeight::get().reads_writes(1, 1);
	...
	fn migration_to_v2_works() {
		new_test_ext().execute_with(|| {
			for i in 0..3u64 {
				...
				fabric_support::migration::put_storage_value(
					b"Scheduler",
					b"Agenda",
					&k,
					old,
