...
fn reward_from_authorship_event_handler_works() {
	ExtBuilder::default().build_and_execute(|| {
		use noble_authorship::EventHandler;

		assert_eq!(<noble_authorship::Module<Test>>::author(), 11);
...
mod offchain_election {
	...
	use fabric_support::traits::OffchainWorker;
	use std::sync::Arc;
	use tetcore_test_utils::assert_eq_uvec;
		...
		ExtBuilder::default()
			...
			.execute_with(|| {
				...
				assert_err_with_weight!(
					...
					Some(<Test as fabric_system::Config>::DbWeight::get().reads(1)),
				);
			})
	}
		...
		ExtBuilder::default()
			...
			.execute_with(|| {
				...
				assert_err_with_weight!(
					...
					Some(<Test as fabric_system::Config>::DbWeight::get().reads(3))
				);
			})
	}
...
fn offences_weight_calculated_correctly() {
	ExtBuilder::default().nominate(true).build_and_execute(|| {
		...
		let zero_offence_weight = <Test as fabric_system::Config>::DbWeight::get().reads_writes(4, 1);
		assert_eq!(Staking::on_offence(&[], &[Perbill::from_percent(50)], 0), Ok(zero_offence_weight));

		// On Offence with N offenders, Unapplied: 4 Reads, 1 Write + 4 Reads, 5 Writes
		let n_offence_unapplied_weight = <Test as fabric_system::Config>::DbWeight::get().reads_writes(4, 1)
			+ <Test as fabric_system::Config>::DbWeight::get().reads_writes(4, 5);

		let offenders: Vec<OffenceDetails<<Test as fabric_system::Config>::AccountId, noble_session::historical::IdentificationTuple<Test>>>
			= (1..10).map(|i|
				OffenceDetails {
		...
		let one_offence_unapplied_weight = <Test as fabric_system::Config>::DbWeight::get().reads_writes(4, 1)
			+ <Test as fabric_system::Config>::DbWeight::get().reads_writes(rw, rw)
			// One `slash_cost`
			+ <Test as fabric_system::Config>::DbWeight::get().reads_writes(6, 5)
			// `slash_cost` * nominators (1)
			+ <Test as fabric_system::Config>::DbWeight::get().reads_writes(6, 5)
			// `reward_cost` * reporters (1)
			+ <Test as fabric_system::Config>::DbWeight::get().reads_writes(2, 2);
...
fn on_initialize_weight_is_correct() {
	ExtBuilder::default().has_stakers(false).build_and_execute(|| {
		...
		// When this noble has nothing, we do 4 reads each block
		let base_weight = <Test as fabric_system::Config>::DbWeight::get().reads(4);
	...
	.execute_with(|| {
		...
		// - (4 + 5) reads
		// - 3 Writes
		let final_weight = <Test as fabric_system::Config>::DbWeight::get().reads_writes(4 + 9, 3);
