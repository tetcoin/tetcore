...
//! [`noble_authorship::EventHandler`](../noble_authorship/trait.EventHandler.html) to add reward
...
//! ## Related Modules
//!
//! - [Balances](../noble_balances/index.html): Used to manage values at stake.
//! - [Session](../noble_session/index.html): Used to manage sessions. Also, a list of new
...
type PositiveImbalanceOf<T> =
	<<T as Config>::Currency as Currency<<T as fabric_system::Config>::AccountId>>::PositiveImbalance;
...
///
/// This is needed because `Staking` sets the `ValidatorIdOf` of the `noble_session::Config`
pub trait SessionInterface<AccountId>: fabric_system::Config {
...
impl<T: Config> SessionInterface<<T as fabric_system::Config>::AccountId> for T where
	T: noble_session::Config<ValidatorId = <T as fabric_system::Config>::AccountId>,
	T: noble_session::historical::Config<
		FullIdentification = Exposure<<T as fabric_system::Config>::AccountId, BalanceOf<T>>,
		FullIdentificationOf = ExposureOf<T>,
	>,
	T::SessionHandler: noble_session::SessionHandler<<T as fabric_system::Config>::AccountId>,
	T::SessionManager: noble_session::SessionManager<<T as fabric_system::Config>::AccountId>,
	T::ValidatorIdOf:
		Convert<<T as fabric_system::Config>::AccountId, Option<<T as fabric_system::Config>::AccountId>>,
{
	fn disable_validator(validator: &<T as fabric_system::Config>::AccountId) -> Result<bool, ()> {
		<noble_session::Module<T>>::disable(validator)
	}

	fn validators() -> Vec<<T as fabric_system::Config>::AccountId> {
		<noble_session::Module<T>>::validators()
	}

	fn prune_historical_up_to(up_to: SessionIndex) {
		<noble_session::historical::Module<T>>::prune_up_to(up_to);
	}
}

pub trait Config: fabric_system::Config + SendTransactionTypes<Call<Self>> {
	/// The staking balance.
	...
	/// [`tp_npos_elections`] crate which accepts u64 numbers and does operations in 128.
	...
	/// multiple nobles send unsigned transactions.
...
pub mod migrations {
	...
	pub fn migrate_to_blockable<T: Config>() -> fabric_support::weights::Weight {
...
impl<T: Config> Module<T> {
	...
	fn new_session(session_index: SessionIndex) -> Option<Vec<T::AccountId>> {
		if let Some(current_era) = Self::current_era() {
			...
			let current_era_start_session_index = Self::eras_start_session_index(current_era)
				.unwrap_or_else(|| {
					fabric_support::print("Error: start_session_index must be set for current_era");
					0
				});
	...
	fn start_session(start_session: SessionIndex) {
			...
			Self::eras_start_session_index(next_active_era)
		{
			if next_active_era_start_session_index == start_session {
				...
				// noble.
				fabric_support::print("Warning: A session appears to have been skipped.");
				Self::start_era(start_session);
			}
		}
	}
	...
	/// Consume a set of [`Supports`] from [`tp_npos_elections`] and collect them into a [`Exposure`]
	fn collect_exposure(
...
impl<T: Config> noble_session::SessionManager<T::AccountId> for Module<T> {
	fn new_session(new_index: SessionIndex) -> Option<Vec<T::AccountId>> {
		fabric_support::debug::native::trace!(
			target: LOG_TARGET,
...
impl<T: Config> historical::SessionManager<T::AccountId, Exposure<T::AccountId, BalanceOf<T>>> for Module<T> {
	...
	{
		<Self as noble_session::SessionManager<_>>::new_session(new_index).map(|validators| {
...
impl<T> noble_authorship::EventHandler<T::AccountId, T::BlockNumber> for Module<T>
	where
		T: Config + noble_authorship::Config + noble_session::Config
{
	...
	fn note_uncle(author: T::AccountId, _age: T::BlockNumber) {
		Self::reward_by_ids(vec![
			(<noble_authorship::Module<T>>::author(), 2),
			(author, 1)
		])
	}
}
...
impl <T: Config>
	OnOffenceHandler<T::AccountId, noble_session::historical::IdentificationTuple<T>, Weight>
for Module<T> where
	T: noble_session::Config<ValidatorId = <T as fabric_system::Config>::AccountId>,
	T: noble_session::historical::Config<
		FullIdentification = Exposure<<T as fabric_system::Config>::AccountId, BalanceOf<T>>,
	...
	T::ValidatorIdOf: Convert<
		...
		Option<<T as fabric_system::Config>::AccountId>,
	>,
{
	fn on_offence(
		offenders: &[OffenceDetails<T::AccountId, noble_session::historical::IdentificationTuple<T>>],
