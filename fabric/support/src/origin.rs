...
macro_rules! impl_outer_origin {
	...
	(
		...
		$crate::impl_outer_origin! {
			$(#[$attr])*
			pub enum $name for $runtime where system = fabric_system {
				$( $rest_without_system )*
			}
		}
	};
	...
	(
		...
		// WARNING: All instance must hold the filter `fabric_system::Config::BaseCallFilter`, except
		// when caller is system Root. One can use `OriginTrait::reset_filter` to do so.
		#[derive(Clone)]
		pub struct $name {
			caller: $caller_name,
			filter: $crate::tetcore_std::rc::Rc<Box<dyn Fn(&<$runtime as $system::Config>::Call) -> bool>>,
		}

		#[cfg(not(feature = "std"))]
		impl $crate::tetcore_std::fmt::Debug for $name {
			fn fmt(
				&self,
				fmt: &mut $crate::tetcore_std::fmt::Formatter
			) -> $crate::tetcore_std::result::Result<(), $crate::tetcore_std::fmt::Error> {
		...
		impl $crate::traits::OriginTrait for $name {
			...
			type NoblesOrigin = $caller_name;
			type AccountId = <$runtime as $system::Config>::AccountId;

			fn add_filter(&mut self, filter: impl Fn(&Self::Call) -> bool + 'static) {
				let f = self.filter.clone();

				self.filter = $crate::tetcore_std::rc::Rc::new(Box::new(move |call| {
					f(call) && filter(call)
				}));
			}
			...
			fn reset_filter(&mut self) {
				...
				self.filter = $crate::tetcore_std::rc::Rc::new(Box::new(filter));
			}
		...
		impl From<$system::Origin<$runtime>> for $name {
			...
			/// * others use `fabric-system::Config::BaseCallFilter`
			fn from(x: $system::Origin<$runtime>) -> Self {
		...
		impl From<$caller_name> for $name {
			fn from(x: $caller_name) -> Self {
				let mut o = $name {
					caller: x,
					filter: $crate::tetcore_std::rc::Rc::new(Box::new(|_| true)),
				};
		...
		impl Into<$crate::tetcore_std::result::Result<$system::Origin<$runtime>, $name>> for $name {
			/// NOTE: converting to noble origin loses the origin filter information.
			fn into(self) -> $crate::tetcore_std::result::Result<$system::Origin<$runtime>, Self> {
		...
		impl From<Option<<$runtime as $system::Config>::AccountId>> for $name {
			...
			/// `fabric-system::Config::BaseCallFilter`.
		...
		$(
			$crate::paste::item! {
				...
				impl From<$module::Origin < $( $generic )? $(, $module::$generic_instance )? > > for $name {
					/// Convert to runtime origin using `fabric-system::Config::BaseCallFilter`.
					fn from(x: $module::Origin < $( $generic )? $(, $module::$generic_instance )? >) -> Self {
				...
				impl Into<
					$crate::tetcore_std::result::Result<
						$module::Origin < $( $generic )? $(, $module::$generic_instance )? >,
						$name,
					>>
				for $name {
					/// NOTE: converting to noble origin loses the origin filter information.
					fn into(self) -> $crate::tetcore_std::result::Result<
						$module::Origin < $( $generic )? $(, $module::$generic_instance )? >,
						Self,
					> {
...
mod tests {
	...
	impl fabric_system::Config for TestRuntime {
		type AccountId = u32;
		type Call = u32;
	...
	impl_outer_origin!(
		pub enum OriginWithSystem for TestRuntime where system = fabric_system {
			origin_without_generic,
			origin_with_generic<T>
		}
	);
	...
	impl_outer_origin!(
		pub enum OriginWithSystem2 for TestRuntime where system = fabric_system {
			origin_with_generic<T>,
			origin_without_generic,
		}
	);
	...
	impl_outer_origin!(
		pub enum OriginEmpty for TestRuntime where system = fabric_system {}
	);

	impl_outer_origin!(
		pub enum OriginIndices for TestRuntime where system = fabric_system, system_index = 11 {
			origin_with_generic<T>,
	...
	fn test_default_filter() {
		...
		assert!(matches!(origin.caller, OriginWithSystemCaller::system(fabric_system::RawOrigin::Root)));
		assert_eq!(origin.filter_call(&0), false);
		assert_eq!(origin.filter_call(&1), false);
