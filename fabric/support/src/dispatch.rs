...
pub use crate::tetcore_std::{result, fmt, prelude::{Vec, Clone, Eq, PartialEq}, marker};
...
pub use crate::traits::{
	CallMetadata, GetCallMetadata, GetCallName, UnfilteredDispatchable, GetNobleVersion,
};

/// The return typ of a `Dispatchable` in fabric. When returned explicitly from
...
/// Serializable version of noble dispatchable.
...
/// ```
/// # #[macro_use]
/// # extern crate fabric_support;
/// # use fabric_support::dispatch;
/// # use fabric_system::{Config, ensure_signed};
/// decl_module! {
...
/// * `Call`: The enum generated for every noble, which implements [`Callable`](./dispatch/trait.Callable.html).
/// * `origin`: Alias of `T::Origin`, declared by the [`impl_outer_origin!`](./macro.impl_outer_origin.html) macro.
/// * `Result`: The expected return type from noble functions.
///
...
/// # #[macro_use]
/// # extern crate fabric_support;
/// # use fabric_support::dispatch::{DispatchResultWithPostInfo, WithPostDispatchInfo};
/// # use fabric_system::{Config, ensure_signed};
/// decl_module! {
/// 	pub struct Module<T: Config> for enum Call where origin: T::Origin {
/// 		#[weight = 1_000_000]
...
/// # #[macro_use]
/// # extern crate fabric_support;
/// # use fabric_support::transactional;
...
/// # extern crate fabric_support;
/// # use fabric_support::dispatch;
/// # use fabric_system::{Config, ensure_signed, ensure_root};
/// decl_module! {
/// 	pub struct Module<T: Config> for enum Call where origin: T::Origin {
/// 		#[weight = 0]
...
/// runtime. For example, the [Balances module](../noble_balances/index.html) can be added multiple times to your
...
/// The default behavior is to call `deposit_event` from the [System module](../fabric_system/index.html).
...
/// Function signature must be `fn on_runtime_upgrade() -> fabric_support::weights::Weight`.
///
...
///   * `fn on_initialize(n: BlockNumber) -> fabric_support::weights::Weight` or
///   * `fn on_initialize() -> fabric_support::weights::Weight`
///
...
/// Function signature can be either:
///   * `fn on_finalize(n: BlockNumber) -> fabric_support::weights::Weight` or
///   * `fn on_finalize() -> fabric_support::weights::Weight`
///
...
macro_rules! decl_module {
	...
	(
		...
		$crate::decl_module!(@normalize
			...
			>
			for enum $call_type where origin: $origin_type, system = fabric_system
	...
	(@impl_deposit_event
		...
		{
			/// Deposits an event using `fabric_system::Module::deposit_event`.
	...
	(@impl_on_runtime_upgrade
			...
			for $module<$trait_instance$(, $instance)?> where $( $other_where_bounds )*
		{
			fn on_runtime_upgrade() -> $return {
				...
				$crate::crate_to_noble_version!()
					.put_into_storage::<<$trait_instance as $system::Config>::NobleInfo, Self>();
	...
	(@create_call_enum
			...
			where $( $other_where_bounds )*
		{
			#[doc(hidden)]
			#[codec(skip)]
			__PhantomItem($crate::tetcore_std::marker::PhantomData<($trait_instance, $($instance)?)>, $crate::Never),
			$( $generated_variants )*
		}
	};
	...
	(@imp
		...
		pub struct $mod_type<
			$trait_instance: $trait_name
			$(<I>, $instance: $instantiable $( = $module_default_instance)?)?
		>($crate::tetcore_std::marker::PhantomData<($trait_instance, $( $instance)?)>) where
			$( $other_where_bounds )*;
		...
		// Bring `GetNobleVersion` into scope to make it easily usable.
		pub use $crate::traits::GetNobleVersion as _;
		// Implement `GetNobleVersion` for `Module`
		impl<$trait_instance: $trait_name $(<I>, $instance: $instantiable)?> $crate::traits::GetNobleVersion
			for $mod_type<$trait_instance $(, $instance)?> where $( $other_where_bounds )*
		{
			fn current_version() -> $crate::traits::NobleVersion {
				$crate::crate_to_noble_version!()
			}

			fn storage_version() -> Option<$crate::traits::NobleVersion> {
				let key = $crate::traits::NobleVersion::storage_key::<
						<$trait_instance as $system::Config>::NobleInfo, Self
			...
			fn get_call_metadata(&self) -> $crate::dispatch::CallMetadata {
				...
				match self {
					$( $call_type::$camelcase(call) => {
						...
						let noble_name = stringify!($camelcase);
						$crate::dispatch::CallMetadata { function_name, noble_name }
					}, )*
				}
			}
		...
		impl $crate::dispatch::Dispatchable for $call_type {
			...
			fn dispatch(
				...
				if !<Self::Origin as $crate::traits::OriginTrait>::filter_call(&origin, &self) {
					return $crate::tetcore_std::result::Result::Err($crate::dispatch::DispatchError::BadOrigin.into())
				}
...
mod tests {
	...
	fn call_metadata() {
		...
		let expected = CallMetadata { function_name: "aux_3".into(), noble_name: "Test".into() };
