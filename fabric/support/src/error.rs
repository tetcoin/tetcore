...
#[doc(hidden)]
pub use tp_runtime::traits::{LookupError, BadOrigin};
#[doc(hidden)]
pub use fabric_metadata::{ModuleErrorMetadata, ErrorMetadata, DecodeDifferent};
...
///
/// ```
/// # use fabric_support::{decl_error, decl_module};
/// #
/// decl_error! {
...
///
///         #[weight = 0]
///         fn do_something(origin) -> fabric_support::dispatch::DispatchResult {
	...
	(
		...
		$( where $( $where_ty: $where_bound ),* )?
		{
			fn from(err: $error<$generic $(, $inst_generic)?>) -> Self {
				let index = <$generic::NobleInfo as $crate::traits::NobleInfo>
