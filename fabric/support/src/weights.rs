...
//! ```
//! # use fabric_system::Config;
//! # use fabric_support::weights::DispatchClass;
//! fabric_support::decl_module! {
//!     pub struct Module<T: Config> for enum Call where origin: T::Origin {
...
//! ```
//! # use fabric_system::Config;
//! # use fabric_support::weights::Pays;
//! fabric_support::decl_module! {
//!     pub struct Module<T: Config> for enum Call where origin: T::Origin {
//!         #[weight = (1000, Pays::No)]
//!         fn dispatching(origin) { unimplemented!() }
...
//! ```
//! # use fabric_system::Config;
//! # use fabric_support::weights::{DispatchClass, Pays};
//! fabric_support::decl_module! {
//!     pub struct Module<T: Config> for enum Call where origin: T::Origin {
...
//! ```
//! # use fabric_system::Config;
//! # use fabric_support::weights::{DispatchClass, FunctionOf, Pays};
//! fabric_support::decl_module! {
//!     pub struct Module<T: Config> for enum Call where origin: T::Origin {
//!         #[weight = FunctionOf(
//! 			// weight, function.
...
//! # fn main() {}
//! ```
//! FABRIC assumes a weight of `1_000_000_000_000` equals 1 second of compute on a standard machine.
//!
...
/// These constants are specific to FABRIC, and the current implementation of its various components.
/// For example: FABRIC System, FABRIC Executive, our FABRIC support libraries, etc...
pub mod constants {
...
/// Primitives related to priority management of Fabric.
pub mod priority {
	...
	pub enum FabricTransactionPriority {
		Normal(u64),
		Operational(u64),
	}

	impl From<FabricTransactionPriority> for u64 {
		fn from(priority: FabricTransactionPriority) -> Self {
			match priority {
				FabricTransactionPriority::Normal(inner) => inner,
				FabricTransactionPriority::Operational(inner) => inner.saturating_add(LIMIT),
			}
		}
	}
}
