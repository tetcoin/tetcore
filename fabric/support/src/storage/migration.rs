...
pub struct StorageIterator<T> {
	...
	previous_key: Vec<u8>,
	drain: bool,
	_phantom: ::tetcore_std::marker::PhantomData<T>,
}
		...
		loop {
			...
			break match maybe_next {
				Some(next) => {
					...
					let maybe_value = fabric_support::storage::unhashed::get::<T>(&next);
					match maybe_value {
						Some(value) => {
							if self.drain {
								fabric_support::storage::unhashed::kill(&next);
							}
...
pub struct StorageKeyIterator<K, T, H: ReversibleStorageHasher> {
	...
	previous_key: Vec<u8>,
	drain: bool,
	_phantom: ::tetcore_std::marker::PhantomData<(K, T, H)>,
}
...
pub fn get_storage_value<T: Decode + Sized>(module: &[u8], item: &[u8], hash: &[u8]) -> Option<T> {
	...
	fabric_support::storage::unhashed::get::<T>(&key)
}
...
pub fn take_storage_value<T: Decode + Sized>(module: &[u8], item: &[u8], hash: &[u8]) -> Option<T> {
	...
	key[16..32].copy_from_slice(&Twox128::hash(item));
	key[32..].copy_from_slice(hash);
	fabric_support::storage::unhashed::take::<T>(&key)
}
...
pub fn put_storage_value<T: Encode>(module: &[u8], item: &[u8], hash: &[u8], value: T) {
	...
	key[16..32].copy_from_slice(&Twox128::hash(item));
	key[32..].copy_from_slice(hash);
	fabric_support::storage::unhashed::put(&key, &value);
}
...
pub fn remove_storage_prefix(module: &[u8], item: &[u8], hash: &[u8]) {
	...
	key[16..32].copy_from_slice(&Twox128::hash(item));
	key[32..].copy_from_slice(hash);
	fabric_support::storage::unhashed::kill_prefix(&key)
}
