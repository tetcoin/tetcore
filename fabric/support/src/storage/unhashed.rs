...
pub fn get<T: Decode + Sized>(key: &[u8]) -> Option<T> {
	tet_io::storage::get(key).and_then(|val| {
...
pub fn put<T: Encode + ?Sized>(key: &[u8], value: &T) {
	value.using_encoded(|slice| tet_io::storage::set(key, slice));
}
...
pub fn exists(key: &[u8]) -> bool {
	tet_io::storage::exists(key)
}

/// Ensure `key` has no explicit entry in storage.
pub fn kill(key: &[u8]) {
	tet_io::storage::clear(key);
}

/// Ensure keys with the given `prefix` have no entries in storage.
pub fn kill_prefix(prefix: &[u8]) {
	tet_io::storage::clear_prefix(prefix);
}

/// Get a Vec of bytes from storage.
pub fn get_raw(key: &[u8]) -> Option<Vec<u8>> {
	tet_io::storage::get(key)
}

/// Put a raw byte slice into storage.
///
/// **WARNING**: If you set the storage of the Tetcore Wasm (`well_known_keys::CODE`),
/// you should also call `fabric_system::RuntimeUpgraded::put(true)` to trigger the
/// `on_runtime_upgrade` logic.
pub fn put_raw(key: &[u8], value: &[u8]) {
	tet_io::storage::set(key, value)
}
