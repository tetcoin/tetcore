...
use crate::noble::Def;
use crate::noble::parse::storage::{Metadata, QueryKind};
...
pub fn expand_storages(def: &mut Def) -> proc_macro2::TokenStream {
		...
		.map(|storage| {
			...
			let metadata_trait = match &storage.metadata {
				Metadata::Value { .. } => quote::quote_spanned!(storage.attr_span =>
					#fabric_support::storage::types::StorageValueMetadata
				),
				Metadata::Map { .. } => quote::quote_spanned!(storage.attr_span =>
					#fabric_support::storage::types::StorageMapMetadata
				),
				Metadata::DoubleMap { .. } => quote::quote_spanned!(storage.attr_span =>
					#fabric_support::storage::types::StorageDoubleMapMetadata
				),
			};
			...
			let ty = match &storage.metadata {
				Metadata::Value { value } => {
					...
					quote::quote_spanned!(storage.attr_span =>
						#fabric_support::metadata::StorageEntryType::Plain(
							#fabric_support::metadata::DecodeDifferent::Encode(#value)
						)
					)
				},
				Metadata::Map { key, value } => {
					...
					quote::quote_spanned!(storage.attr_span =>
						#fabric_support::metadata::StorageEntryType::Map {
							hasher: <#full_ident as #metadata_trait>::HASHER,
							key: #fabric_support::metadata::DecodeDifferent::Encode(#key),
							value: #fabric_support::metadata::DecodeDifferent::Encode(#value),
							unused: false,
						}
					)
				},
				Metadata::DoubleMap { key1, key2, value } => {
					...
					quote::quote_spanned!(storage.attr_span =>
						#fabric_support::metadata::StorageEntryType::DoubleMap {
							hasher: <#full_ident as #metadata_trait>::HASHER1,
							key2_hasher: <#full_ident as #metadata_trait>::HASHER2,
							key1: #fabric_support::metadata::DecodeDifferent::Encode(#key1),
							key2: #fabric_support::metadata::DecodeDifferent::Encode(#key2),
							value: #fabric_support::metadata::DecodeDifferent::Encode(#value),
						}
					)
				}
			...
			quote::quote_spanned!(storage.attr_span =>
				#fabric_support::metadata::StorageEntryMetadata {
					name: #fabric_support::metadata::DecodeDifferent::Encode(
						<#full_ident as #metadata_trait>::NAME
					),
					modifier: <#full_ident as #metadata_trait>::MODIFIER,
					ty: #ty,
					default: #fabric_support::metadata::DecodeDifferent::Encode(
						<#full_ident as #metadata_trait>::DEFAULT
					),
					documentation: #fabric_support::metadata::DecodeDifferent::Encode(&[
						#( #docs, )*
					]),
				}
			)
				...
				Metadata::Value { value } => {
					...
					quote::quote_spanned!(storage.attr_span =>
						impl<#type_impl_gen> #noble_ident<#type_use_gen> #completed_where_clause {
							...
							pub fn #getter() -> #query {
								<
									#full_ident as #fabric_support::storage::StorageValue<#value>
								>::get()
							}
						}
					)
					...
					quote::quote_spanned!(storage.attr_span =>
						impl<#type_impl_gen> #noble_ident<#type_use_gen> #completed_where_clause {
							...
							pub fn #getter<KArg>(k: KArg) -> #query where
								KArg: #fabric_support::codec::EncodeLike<#key>,
							{
								<
									#full_ident as #fabric_support::storage::StorageMap<#key, #value>
								>::get(k)
							}
						}
					)
					...
					quote::quote_spanned!(storage.attr_span =>
						impl<#type_impl_gen> #noble_ident<#type_use_gen> #completed_where_clause {
							...
							pub fn #getter<KArg1, KArg2>(k1: KArg1, k2: KArg2) -> #query where
								KArg1: #fabric_support::codec::EncodeLike<#key1>,
								KArg2: #fabric_support::codec::EncodeLike<#key2>,
							{
								<
									#full_ident as
									#fabric_support::storage::StorageDoubleMap<#key1, #key2, #value>
								>::get(k1, k2)
							}
						}
					)
	...
	let prefix_structs = def.storages.iter().map(|storage_def| {
		...
		quote::quote_spanned!(storage_def.attr_span =>
			...
			impl<#type_impl_gen> #fabric_support::traits::StorageInstance
	...
	quote::quote!(
		impl<#type_impl_gen> #noble_ident<#type_use_gen>
			#completed_where_clause
		{
			#[doc(hidden)]
			pub fn storage_metadata() -> #fabric_support::metadata::StorageMetadata {
				#fabric_support::metadata::StorageMetadata {
					prefix: #fabric_support::metadata::DecodeDifferent::Encode(
						<
							<T as #fabric_system::Config>::NobleInfo as
							#fabric_support::traits::NobleInfo
						>::name::<#noble_ident<#type_use_gen>>()
							.expect("Every active noble has a name in the runtime; qed")
					),
					entries: #fabric_support::metadata::DecodeDifferent::Encode(
						&[ #( #entries, )* ]
					),
				}
			}
