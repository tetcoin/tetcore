...
pub fn expand_event(def: &mut Def) -> proc_macro2::TokenStream {
	...
	let metadata = event.metadata.iter()
		.map(|(ident, args, docs)| {
			...
			quote::quote_spanned!(event.attr_span =>
				#fabric_support::event::EventMetadata {
					name: #fabric_support::event::DecodeDifferent::Encode(#name),
					arguments: #fabric_support::event::DecodeDifferent::Encode(&[
						#( #args, )*
					]),
					documentation: #fabric_support::event::DecodeDifferent::Encode(&[
						#( #docs, )*
					]),
				},
			)
	...
	if event.gen_kind.is_generic() {
		let variant = syn::parse_quote!(
			...
			#[codec(skip)]
			__Ignore(
				#fabric_support::tetcore_std::marker::PhantomData<(#event_use_gen)>,
	...
	let deposit_event = if let Some((fn_vis, fn_span)) = &event.deposit_event {
		...
		quote::quote_spanned!(*fn_span =>
			impl<#type_impl_gen> Noble<#type_use_gen> #completed_where_clause {
				#fn_vis fn deposit_event(event: Event<#event_use_gen>) {
					...
					let event = <
						<T as Config#trait_use_gen>::Event as
						Into<<T as #fabric_system::Config>::Event>
					>::into(event);

					<#fabric_system::Noble<T>>::deposit_event(event)
				}
			}
		)
	} else {
	...
	quote::quote_spanned!(event.attr_span =>
		...
		impl<#event_impl_gen> #event_ident<#event_use_gen> #event_where_clause {
			#[allow(dead_code)]
			#[doc(hidden)]
			pub fn metadata() -> &'static [#fabric_support::event::EventMetadata] {
				&[ #( #metadata )* ]
			}
		}
	)
