...
/// If attribute `#[noble::generate_store(..)]` is defined then:
/// * generate Store trait with all storages,
/// * implement Store trait for Noble.
pub fn expand_store_trait(def: &mut Def) -> proc_macro2::TokenStream {
	let (trait_vis, trait_store) = if let Some(store) = &def.noble_struct.store {
		store
	} else {
	...
	quote::quote_spanned!(trait_store.span() =>
		...
		impl<#type_impl_gen> #trait_store for #noble_ident<#type_use_gen>
