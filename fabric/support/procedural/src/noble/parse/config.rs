...
mod keyword {
	...
	syn::custom_keyword!(fabric_system);
	syn::custom_keyword!(disable_fabric_system_supertrait_check);
}

/// Input definition for the noble config.
pub struct ConfigDef {
	/// The index of item in noble module.
	pub index: usize,
	...
	/// * `IsType<Self as fabric_system::Config>::Event`
	...
	/// The span of the noble::config attribute.
	pub attr_span: proc_macro2::Span,
}

/// Input definition for a constant in noble config.
...
/// Parse for `#[noble::disable_fabric_system_supertrait_check]`
pub struct DisableFabricSystemSupertraitCheck;

impl syn::parse::Parse for DisableFabricSystemSupertraitCheck {
	fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
		input.parse::<syn::Token![#]>()?;
		let content;
		...
		content.parse::<keyword::disable_fabric_system_supertrait_check>()?;
		Ok(Self)
	}
}

/// Parse for `#[noble::constant]`
		...
		if type_.ident == "Event" {
			...
			let has_is_type_bound = type_.bounds.iter().any(|s| {
				syn::parse2::<IsTypeBoundEventParse>(s.to_token_stream())
					.map_or(false, |b| b.0 == *fabric_system)
			});
...
impl ConfigDef {
	pub fn try_from(
		...
		let item = if let syn::Item::Trait(item) = item {
			...
			let msg = "Invalid noble::config, expected trait definition";
			return Err(syn::Error::new(item.span(), msg));
		};

		if !matches!(item.vis, syn::Visibility::Public(_)) {
			let msg = "Invalid noble::config, trait must be public";
			return Err(syn::Error::new(item.span(), msg));
		}
		...
		if item.generics.params.len() > 1 {
			let msg = "Invalid noble::config, expected no more than one generic";
		...
		for trait_item in &mut item.items {
			...
			has_event_type = has_event_type
				|| check_event_type(fabric_system, trait_item, has_instance)?;

			// Parse for constant
			let type_attrs_const: Vec<TypeAttrConst> = helper::take_item_attrs(trait_item)?;

			if type_attrs_const.len() > 1 {
				let msg = "Invalid attribute in noble::config, only one attribute is expected";
				...
				match trait_item {
					syn::TraitItem::Type(type_) => {
						let constant = syn::parse2::<ConstMetadataDef>(type_.to_token_stream())
							.map_err(|e| {
								let error_msg = "Invalid usage of `#[noble::constant]`, syntax \
					...
					_ => {
						let msg = "Invalid noble::constant in noble::config, expected type trait \
							item";
						return Err(syn::Error::new(trait_item.span(), msg));
					},
				}
		...
		let attr: Option<DisableFabricSystemSupertraitCheck> = helper::take_first_item_attr(
			&mut item.attrs
		)?;

		let disable_system_supertrait_check = attr.is_some();

		let has_fabric_system_supertrait = item.supertraits.iter().any(|s| {
			syn::parse2::<ConfigBoundParse>(s.to_token_stream())
				.map_or(false, |b| b.0 == *fabric_system)
		});

		if !has_fabric_system_supertrait && !disable_system_supertrait_check {
			...
			let msg = format!(
				"Invalid noble::trait, expected explicit `{}::Config` as supertrait, \
				found {}. \
				(try `pub trait Config: fabric_system::Config {{ ...` or \
				`pub trait Config<I: 'static>: fabric_system::Config {{ ...`). \
				To disable this check, use `#[noble::disable_fabric_system_supertrait_check]`",
				fabric_system,
				found,
			);
			return Err(syn::Error::new(item.span(), msg));
