...
/// Parse for `#[noble::getter(fn dummy)]`
pub struct NobleStorageAttr {
	getter: syn::Ident,
}

impl syn::parse::Parse for NobleStorageAttr {
...
pub struct StorageDef {
	/// The index of error item in noble module.
	pub index: usize,
	...
	/// The span of the noble::storage attribute.
...
fn retrieve_arg(
	...
	if let syn::PathArguments::AngleBracketed(args) = &segment.arguments {
		if arg_pos < args.args.len() {
			...
			let msg = format!("noble::storage unexpected number of generic argument, expected at \
...
impl StorageDef {
	pub fn try_from(
		...
		let item = if let syn::Item::Type(item) = item {
			...
			return Err(syn::Error::new(item.span(), "Invalid noble::storage, expected item type"));
		};

		let mut attrs: Vec<NobleStorageAttr> = helper::take_item_attrs(&mut item.attrs)?;
		if attrs.len() > 1 {
			let msg = "Invalid noble::storage, multiple argument noble::getter found";
		...
		let typ = if let syn::Type::Path(typ) = &*item.ty {
			...
			let msg = "Invalid noble::storage, expected type path";
			return Err(syn::Error::new(item.ty.span(), msg));
		};

		if typ.path.segments.len() != 1 {
			let msg = "Invalid noble::storage, expected type path with one segment";
			return Err(syn::Error::new(item.ty.span(), msg));
		}
		...
		let metadata = match &*typ.path.segments[0].ident.to_string() {
			...
			found => {
				let msg = format!(
					"Invalid noble::storage, expected ident: `StorageValue` or \
		...
		if query_kind.is_none() && getter.is_some() {
			let msg = "Invalid noble::storage, cannot generate getter because QueryKind is not \
		...
		syn::parse2::<syn::Token![_]>(prefix_arg.to_token_stream())
			.map_err(|e| {
				let msg = "Invalid use of `#[noble::storage]`, the type first generic argument \
