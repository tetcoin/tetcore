...
/// Take the first noble attribute (e.g. attribute like `#[noble..]`) and decode it to `Attr`
	...
	Attr: syn::parse::Parse,
{
	...
	if let Some(index) = attrs.iter()
		.position(|attr|
			attr.path.segments.first().map_or(false, |segment| segment.ident == "noble")
		)
	{
		let noble_attr = attrs.remove(index);
		Ok(Some(syn::parse2(noble_attr.into_token_stream())?))
	} else {
		Ok(None)
	}
}

/// Take all the noble attributes (e.g. attribute like `#[noble..]`) and decode them to `Attr`
pub fn take_item_attrs<Attr>(item: &mut impl MutItemAttrs) -> syn::Result<Vec<Attr>> where
	Attr: syn::parse::Parse,
{
	let mut noble_attrs = Vec::new();

	while let Some(attr) = take_first_item_attr(item)? {
		noble_attrs.push(attr)
	}
...
///
/// return the instance if found.
pub fn check_noble_struct_usage(type_: &Box<syn::Type>) -> syn::Result<InstanceUsage> {
	let expected = "expected `Noble<T>` or `Noble<T, I>`";
	pub struct Checker(InstanceUsage);
	impl syn::parse::Parse for Checker {
		fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
			let mut instance_usage = InstanceUsage {
	...
	let i = syn::parse2::<Checker>(type_.to_token_stream())
		.map_err(|e| {
			let msg = format!("Invalid noble struct: {}", expected);
