...
pub struct ErrorDef {
	...
	/// The span of the noble::error attribute.
...
impl ErrorDef {
	pub fn try_from(
		...
		let item = if let syn::Item::Enum(item) = item {
			...
			return Err(syn::Error::new(item.span(), "Invalid noble::error, expected item enum"));
		};
		if !matches!(item.vis, syn::Visibility::Public(_)) {
			let msg = "Invalid noble::error, `Error` must be public";
		...
		if item.generics.where_clause.is_some() {
			let msg = "Invalid noble::error, where clause is not allowed on noble error item";
		...
		let variants = item.variants.iter()
			.map(|variant| {
				if !matches!(variant.fields, syn::Fields::Unit) {
					let msg = "Invalid noble::error, unexpected fields, must be `Unit`";
					return Err(syn::Error::new(variant.fields.span(), msg));
				}
				if variant.discriminant.is_some() {
					let msg = "Invalid noble::error, unexpected discriminant, discriminant \
						are not supported";
