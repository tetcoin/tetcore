...
/// Definition for noble event enum.
pub struct EventDef {
	/// The index of event item in noble module.
	pub index: usize,
	...
	/// The span of the noble::event attribute.
...
///
/// Syntax is:
/// * `#[noble::metadata(SomeType = MetadataName, ...)]`
/// * `#[noble::generate_deposit($vis fn deposit_event)]`
enum NobleEventAttr {
	Metadata {
...
impl syn::parse::Parse for NobleEventAttr {
	fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
		...
		if lookahead.peek(keyword::metadata) {
			...
			Ok(NobleEventAttr::Metadata { metadata, span })
			...
			Ok(NobleEventAttr::DepositEvent { fn_vis, span, fn_span })
		} else {
			Err(lookahead.error())
		}
	}
...
struct NobleEventAttrInfo {
	metadata: Option<Vec<(syn::Type, String)>>,
	deposit_event: Option<(syn::Visibility, proc_macro2::Span)>,
}

impl NobleEventAttrInfo {
	fn from_attrs(attrs: Vec<NobleEventAttr>) -> syn::Result<Self> {
		let mut metadata = None;
		let mut deposit_event = None;
		for attr in attrs {
			match attr {
				NobleEventAttr::Metadata { metadata: m, .. } if metadata.is_none() =>
					metadata = Some(m),
				NobleEventAttr::DepositEvent { fn_vis, fn_span, .. } if deposit_event.is_none() =>
		...
		Ok(NobleEventAttrInfo { metadata, deposit_event })
	}
}
...
impl EventDef {
	pub fn try_from(
		...
		let item = if let syn::Item::Enum(item) = item {
			...
			return Err(syn::Error::new(item.span(), "Invalid noble::event, expected item enum"))
		};

		let event_attrs: Vec<NobleEventAttr> = helper::take_item_attrs(&mut item.attrs)?;
		let attr_info = NobleEventAttrInfo::from_attrs(event_attrs)?;
		let metadata = attr_info.metadata.unwrap_or_else(Vec::new);
		let deposit_event = attr_info.deposit_event;

		if !matches!(item.vis, syn::Visibility::Public(_)) {
			let msg = "Invalid noble::event, `Error` must be public";
		...
		if let Some(u) = helper::check_type_def_optional_gen(&item.generics, item.ident.span())? {
			...
			// construct_runtime only allow non generic event for non instantiable noble.
