...
/// Definition for noble genesis config type.
///
/// Either:
...
impl GenesisConfigDef {
	pub fn try_from(index: usize, item: &mut syn::Item) -> syn::Result<Self> {
		...
		let (vis, ident, generics) = match &item {
			...
			_ => {
				let msg = "Invalid noble::genesis_config, expected enum or struct";
		...
		if !matches!(vis, syn::Visibility::Public(_)) {
			let msg = "Invalid noble::genesis_config, GenesisConfig must be public";
			return Err(syn::Error::new(item_span, msg));
		}

		if ident != "GenesisConfig" {
			let msg = "Invalid noble::genesis_config, ident must `GenesisConfig`";
