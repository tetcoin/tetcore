...
/// Definition of dispatchables typically `impl<T: Config> Noble<T> { ... }`
pub struct CallDef {
	/// The where_clause used.
	pub where_clause: Option<syn::WhereClause>,
	/// A set of usage of instance, must be check for consistency with trait.
	pub instances: Vec<helper::InstanceUsage>,
	/// The index of call item in noble module.
	pub index: usize,
	/// Information on methods (used for expansion).
	pub methods: Vec<CallVariantDef>,
	/// The span of the noble::call attribute.
...
impl CallDef {
	pub fn try_from(
		...
		let item = if let syn::Item::Impl(item) = item {
			...
			return Err(syn::Error::new(item.span(), "Invalid noble::call, expected item impl"));
		};

		let mut instances = vec![];
		instances.push(helper::check_impl_gen(&item.generics, item.impl_token.span())?);
		instances.push(helper::check_noble_struct_usage(&item.self_ty)?);

		if let Some((_, _, for_)) = item.trait_ {
			let msg = "Invalid noble::call, expected no trait ident as in \
				`impl<..> Noble<..> { .. }`";
		...
		for impl_item in &mut item.items {
			if let syn::ImplItem::Method(method) = impl_item {
				match method.sig.inputs.first() {
					None => {
						let msg = "Invalid noble::call, must have at least origin arg";
						return Err(syn::Error::new(method.sig.span(), msg));
					},
					Some(syn::FnArg::Receiver(_)) => {
						let msg = "Invalid noble::call, first argument must be a typed argument, \
				...
				if let syn::ReturnType::Type(_, type_) = &method.sig.output {
					...
					let msg = "Invalid noble::call, require return type \
				...
				if call_var_attrs.len() != 1 {
					let msg = if call_var_attrs.is_empty() {
						"Invalid noble::call, requires weight attribute i.e. `#[noble::weight($expr)]`"
					} else {
						"Invalid noble::call, too many weight attributes given"
					};
					return Err(syn::Error::new(method.sig.span(), msg));
				}
				...
				for arg in method.sig.inputs.iter_mut().skip(1) {
					...
					if arg_attrs.len() > 1 {
						let msg = "Invalid noble::call, argument has too many attributes";
					...
					let arg_ident = if let syn::Pat::Ident(pat) = &*arg.pat {
						...
						let msg = "Invalid noble::call, argument must be ident";
				...
				let msg = "Invalid noble::call, only method accepted";
