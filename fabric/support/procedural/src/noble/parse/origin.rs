...
/// Definition of the noble origin type.
///
/// Either:
/// * `type Origin`
/// * `struct Origin`
...
impl OriginDef {
	pub fn try_from(index: usize, item: &mut syn::Item) -> syn::Result<Self> {
		...
		let (vis, ident, generics) = match &item {
			...
			_ => {
				let msg = "Invalid noble::origin, expected enum or struct or type";
		...
		if let Some(u) = helper::check_type_def_optional_gen(&generics, item.span())? {
			...
			// construct_runtime only allow generic event for instantiable noble.
		...
		if !matches!(vis, syn::Visibility::Public(_)) {
			let msg = "Invalid noble::origin, Origin must be public";
			return Err(syn::Error::new(item_span, msg));
		}

		if ident != "Origin" {
			let msg = "Invalid noble::origin, ident must `Origin`";
