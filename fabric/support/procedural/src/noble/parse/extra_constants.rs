...
/// Definition of extra constants typically `impl<T: Config> Noble<T> { ... }`
...
/// Input definition for an constant in noble.
	...
	pub fn try_from(
		...
		for impl_item in &mut item.items {
			...
			if !method.sig.inputs.is_empty() {
				let msg = "Invalid noble::extra_constants, method must have 0 args";
				return Err(syn::Error::new(method.sig.span(), msg));
			}

			if !method.sig.generics.params.is_empty() {
				let msg = "Invalid noble::extra_constants, method must have 0 generics";
				return Err(syn::Error::new(method.sig.generics.params[0].span(), msg));
			}

			if method.sig.generics.where_clause.is_some() {
				let msg = "Invalid noble::extra_constants, method must have no where clause";
				return Err(syn::Error::new(method.sig.generics.where_clause.span(), msg));
			}

			let type_ = match &method.sig.output {
				syn::ReturnType::Default => {
					let msg = "Invalid noble::extra_constants, method must have a return type";
