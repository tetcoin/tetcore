...
impl TypeValueDef {
	pub fn try_from(
		...
		let item = if let syn::Item::Fn(item) = item {
			...
			let msg = "Invalid noble::type_value, expected item fn";
			return Err(syn::Error::new(item.span(), msg));
		};


		if !item.attrs.is_empty() {
			let msg = "Invalid noble::type_value, unexpected attribute";
		...
		{
			let msg = "Invalid noble::type_value, unexpected token";
			return Err(syn::Error::new(span, msg));
		}

		if !item.sig.inputs.is_empty() {
			let msg = "Invalid noble::type_value, unexpected argument";
		...
		let type_ = match item.sig.output.clone() {
			...
			syn::ReturnType::Default => {
				let msg = "Invalid noble::type_value, expected return type";
