...
use fabric_support_procedural_tools::syn_ext as ext;
use fabric_support_procedural_tools::{generate_crate_access, generate_hidden_includes};
...
fn construct_runtime_parsed(definition: RuntimeDefinition) -> Result<TokenStream2> {
	...
	let system_module = modules.iter()
		...
		.ok_or_else(|| syn::Error::new(
			...
			"`System` module declaration is missing. \
			 Please add this line: `System: fabric_system::{Module, Call, Storage, Config, Event<T>},`",
		))?;

	let hidden_crate_name = "construct_runtime";
	let scrate = generate_crate_access(&hidden_crate_name, "fabric-support");
	let scrate_decl = generate_hidden_includes(&hidden_crate_name, "fabric-support");
	...
	let module_to_index = decl_noble_runtime_setup(&modules, &scrate);
...
fn decl_noble_runtime_setup(
	...
	quote!(
		/// Provides an implementation of `NobleInfo` to provide information
		/// about the noble setup in the runtime.
		pub struct NobleInfo;

		impl #scrate::traits::NobleInfo for NobleInfo {
			fn index<P: 'static>() -> Option<usize> {
				let type_id = #scrate::tetcore_std::any::TypeId::of::<P>();
				#(
					if type_id == #scrate::tetcore_std::any::TypeId::of::<#names>() {
						return Some(#indices)
					}
				)*
			...
			fn name<P: 'static>() -> Option<&'static str> {
				let type_id = #scrate::tetcore_std::any::TypeId::of::<P>();
				#(
					if type_id == #scrate::tetcore_std::any::TypeId::of::<#names2>() {
