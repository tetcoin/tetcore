...
/// Environment variable that tells us to print noble upgrade helper.
const PRINT_NOBLE_UPGRADE: &str = "PRINT_NOBLE_UPGRADE";

fn check_print_noble_upgrade() -> bool {
	std::env::var(PRINT_NOBLE_UPGRADE).is_ok()
}
...
/// Print an incomplete upgrade from decl_storage macro to new noble attribute.
pub fn maybe_print_noble_upgrade(def: &super::DeclStorageDefExt) {
	if !check_print_noble_upgrade() {
		return
	}

	let scrate = &quote::quote!(fabric_support);
...
{default_value_type_value}{doc}
	...
	println!("
// Template for noble upgrade for {noble_name}
...
pub mod noble {{
	...
	#[noble::config]
	pub trait Config{config_gen}: fabric_system::Config
	...
	#[noble::noble]
	#[noble::generate_store({store_vis} trait Store)]
	pub struct Noble{decl_gen}(PhantomData{use_gen_tuple});

	#[noble::hooks]
	impl{impl_gen} Hooks<BlockNumberFor<T>> for Noble{use_gen}
		// TODO_MAYBE_WHERE_CLAUSE
	{{
		// TODO_ON_FINALIZE
		// TODO_ON_INITIALIZE
