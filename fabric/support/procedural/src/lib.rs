...
///   [`StorageValue`](../fabric_support/storage/trait.StorageValue.html) trait using the
///   [`StorageValue generator`](../fabric_support/storage/generator/trait.StorageValue.html).
///
...
///   [`StorageMap`](../fabric_support/storage/trait.StorageMap.html) trait using the
///   [`StorageMap generator`](../fabric_support/storage/generator/trait.StorageMap.html).
///   And [`StoragePrefixedMap`](../fabric_support/storage/trait.StoragePrefixedMap.html).
///
///   `$hash` representing a choice of hashing algorithms available in the
///   [`Hashable`](../fabric_support/trait.Hashable.html) trait. You will generally want to use one
///   of three hashers:
...
///   [`StorageDoubleMap`](../fabric_support/storage/trait.StorageDoubleMap.html) trait using the
///   [`StorageDoubleMap generator`](../fabric_support/storage/generator/trait.StorageDoubleMap.html).
///   And [`StoragePrefixedMap`](../fabric_support/storage/trait.StoragePrefixedMap.html).
///
///   `$hash1` and `$hash2` representing choices of hashing algorithms available in the
///   [`Hashable`](../fabric_support/trait.Hashable.html) trait. They must be chosen with care, see
...
/// ```
///
/// The identifier `System` is the name of the noble and the lower case identifier `system` is the
/// name of the Rust module/crate for this Tetcore module. The identifiers between the braces are
/// the module parts provided by the noble. It is important to list these parts here to export
/// them correctly in the metadata or to make the noble usable in the runtime.
///
...
/// # Type definitions
///
/// * The macro generates a type alias for each noble to their `Module` (or `Noble`).
///   E.g. `type System = fabric_system::Module<Runtime>`
#[proc_macro]
...
pub fn noble(attr: TokenStream, item: TokenStream) -> TokenStream {
	noble::noble(attr, item)
}
...
/// Derive [`Debug`], if `std` is enabled it uses `fabric_support::DebugNoBound`, if `std` is not
...
#[proc_macro]
pub fn crate_to_noble_version(input: TokenStream) -> TokenStream {
	noble_version::crate_to_noble_version(input).unwrap_or_else(|e| e.to_compile_error()).into()
}
