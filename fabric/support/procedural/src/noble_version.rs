...
//! Implementation of macros related to noble versioning.
...
/// Implementation of the `crate_to_noble_version!` macro.
pub fn crate_to_noble_version(input: proc_macro::TokenStream) -> Result<TokenStream> {
	if !input.is_empty() {
	...
	let crate_ = generate_crate_access_2018("fabric-support")?;

	Ok(quote::quote! {
		#crate_::traits::NobleVersion {
			major: #major_version,
			minor: #minor_version,
			patch: #patch_version,
		}
