...
pub trait SomeAssociation {
	type A: fabric_support::dispatch::Parameter + Default;
}
...
pub mod noble {
	...
	#[noble::hooks]
	impl<T: Config> Hooks<T::BlockNumber> for Noble<T> {
		fn on_initialize(_n: T::BlockNumber) -> Weight {
			<Dummy<T>>::put(T::Balance::from(10));
			10
		}
	...
	#[noble::type_value] pub fn OnFooEmpty<T: Config>() -> T::Balance { 3.into() }
	#[noble::storage]
