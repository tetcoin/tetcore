...
mod tests {
	...
	impl fabric_support_test::Config for TraitImpl {
		type Origin = u32;
...
mod test_append_and_len {
	...
	impl fabric_support_test::Config for Test {
		type Origin = u32;
		type BlockNumber = u32;
		type NobleInfo = ();
		type DbWeight = ();
	...
	fn append_overwrites_invalid_data() {
		TestExternalities::default().execute_with(|| {
			...
			fabric_support::storage::unhashed::put_raw(&key, &*b"1");
			assert!(JustVec::get().is_empty());
			assert_eq!(fabric_support::storage::unhashed::get_raw(&key), Some(b"1".to_vec()));
