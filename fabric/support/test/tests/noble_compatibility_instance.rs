...
mod noble_old {
	...
	pub trait Config<I: Instance = DefaultInstance>: fabric_system::Config {
...
pub mod noble {
	...
	#[noble::hooks]
	impl<T: Config<I>, I: 'static> Hooks<T::BlockNumber> for Noble<T, I> {
		fn on_initialize(_n: T::BlockNumber) -> Weight {
			<Dummy<T, I>>::put(T::Balance::from(10));
			10
		}
	...
	impl<T: Config<I>, I: 'static> Noble<T, I> {
		#[noble::weight(<T::Balance as Into<Weight>>::into(new_value.clone()))]
		fn set_dummy(
			origin: OriginFor<T>,
			#[noble::compact] new_value: T::Balance
	...
	#[noble::metadata(T::Balance = "Balance")]
	...
	#[noble::type_value] pub fn OnFooEmpty<T: Config<I>, I: 'static>() -> T::Balance { 3.into() }
...
impl noble::Config<noble::Instance2> for Runtime {
	type Event = Event;
	type SomeConst = SomeConst;
	type Balance = u64;
}
impl noble::Config<noble::Instance3> for Runtime {
	type Event = Event;
	type SomeConst = SomeConst;
	type Balance = u64;
}
impl noble_old::Config for Runtime {
	type Event = Event;
	type SomeConst = SomeConst;
	type Balance = u64;
}
impl noble_old::Config<noble_old::Instance2> for Runtime {
	type Event = Event;
	type SomeConst = SomeConst;
	type Balance = u64;
}
impl noble_old::Config<noble_old::Instance3> for Runtime {
	type Event = Event;
	type SomeConst = SomeConst;
	type Balance = u64;
}
...
fabric_support::construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		Example: noble::{Module, Call, Event<T>, Config<T>, Storage},
		NobleOld: noble_old::{Module, Call, Event<T>, Config<T>, Storage},
		Instance2Example: noble::<Instance2>::{Module, Call, Event<T>, Config<T>, Storage},
		NobleOld2: noble_old::<Instance2>::{Module, Call, Event<T>, Config<T>, Storage},
		Instance3Example: noble::<Instance3>::{Module, Call, Event<T>, Config<T>, Storage},
		NobleOld3: noble_old::<Instance3>::{Module, Call, Event<T>, Config<T>, Storage},
	}
);
...
mod test {
	...
	fn metadata() {
		...
		let modules = match metadata.1 {
			fabric_metadata::RuntimeMetadata::V12(fabric_metadata::RuntimeMetadataV12 {
				modules: fabric_metadata::DecodeDifferent::Encode(m),
				..
			}) => m,
			_ => unreachable!(),
		};
	...
	fn types() {
		assert_eq!(
			noble_old::Event::<Runtime>::decode(
				&mut &noble::Event::<Runtime>::Dummy(10).encode()[..]
			).unwrap(),
			noble_old::Event::<Runtime>::Dummy(10),
		);

		assert_eq!(
			noble_old::Call::<Runtime>::decode(
				&mut &noble::Call::<Runtime>::set_dummy(10).encode()[..]
			).unwrap(),
			noble_old::Call::<Runtime>::set_dummy(10),
		);
	}
}
