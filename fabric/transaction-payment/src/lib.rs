...
impl<T, S, V, M> MultiplierUpdate for TargetedFeeAdjustment<T, S, V, M>
	where T: fabric_system::Config, S: Get<Perquintill>, V: Get<Multiplier>, M: Get<Multiplier>,
{
	fn min() -> Multiplier {
		M::get()
	}
	...
	where T: fabric_system::Config, S: Get<Perquintill>, V: Get<Multiplier>, M: Get<Multiplier>,
{
	fn convert(previous: Multiplier) -> Multiplier {
		...
		let current_block_weight = <fabric_system::Module<T>>::block_weight();
	...
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		fn integrity_test() {
			...
			tet_io::TestExternalities::new_empty().execute_with(|| {
				<fabric_system::Module<T>>::set_block_consumed_resources(target, 0);
...
mod tests {
	use super::*;
	use crate as noble_transaction_payment;
	...
	type UncheckedExtrinsic = fabric_system::mocking::MockUncheckedExtrinsic<Runtime>;
	type Block = fabric_system::mocking::MockBlock<Runtime>;
	...
	const CALL: &<Runtime as fabric_system::Config>::Call =
	...
	impl Get<fabric_system::limits::BlockWeights> for BlockWeights {
		fn get() -> fabric_system::limits::BlockWeights {
	...
	fn signed_extension_transaction_payment_is_bounded() {
			...
			.execute_with(||
		{
			...
			assert_eq!(
				...
				(10000 - <Runtime as fabric_system::Config>::BlockWeights::get().max_block) as u64
			);
		});
	}
	...
	fn refund_does_not_recreate_account() {
			...
			.execute_with(||
		{
			...
			assert!(System::events().iter().any(|event| {
				event.event == Event::noble_balances(noble_balances::RawEvent::Transfer(2, 3, 80))
			}));
			// Killed Event
			assert!(System::events().iter().any(|event| {
