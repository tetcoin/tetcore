...
pub trait Config: fabric_system::Config {
	...
	/// A dispatchable call.
	type Call: Parameter + Dispatchable<Origin=Self::Origin> + GetDispatchInfo + From<fabric_system::Call<Self>>;
...
// Any runtime call can be encoded into two bytes which represent the noble and call index.
...
decl_storage! {
	trait Store for Module<T: Config> as Lottery {
		...
		/// The calls stored in this noble to be used in an active lottery if configured
...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin, system = fabric_system {
		...
		/// provided by this noble, which uses storage to determine the valid calls.
		///
		...
		fn start_lottery(origin,
			...
			Lottery::<T>::try_mutate(|lottery| -> DispatchResult {
				...
				let start = fabric_system::Module::<T>::block_number();
