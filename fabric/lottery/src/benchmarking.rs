...
use fabric_support::traits::{OnInitialize, UnfilteredDispatchable};
use fabric_benchmarking::{benchmarks, account, whitelisted_caller};
use tp_runtime::traits::{Bounded, Zero};
...
fn setup_lottery<T: Config>(repeat: bool) -> Result<(), &'static str> {
	...
	let mut calls = vec![
		fabric_system::Call::<T>::set_code(vec![]).into();
		T::MaxCalls::get().saturating_sub(1)
	];
	// Last call will be the match for worst case scenario.
	calls.push(fabric_system::Call::<T>::remark(vec![]).into());
...
benchmarks! {
	buy_ticket {
		...
		let call = fabric_system::Call::<T>::remark(vec![]);
	...
	set_calls {
		...
		let calls = vec![fabric_system::Call::<T>::remark(vec![]).into(); n as usize];
