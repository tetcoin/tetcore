...
//! documentation.
//!
//! - [`noble_example_offchain_worker::Config`](./trait.Config.html)
//! - [`Call`](./enum.Call.html)
...
/// Based on the above `KeyTypeId` we need to generate a noble-specific crypto type wrappers.
/// We can use from supported crypto kinds (`sr25519`, `ed25519` and `ecdsa`) and augment
/// the types with this noble-specific identifier.
pub mod crypto {
	use super::KEY_TYPE;
	use tp_runtime::{
	...
	impl fabric_system::offchain::AppCrypto<<Sr25519Signature as Verify>::Signer, Sr25519Signature> for TestAuthId {
...
decl_event!(
	...
	pub enum Event<T> where AccountId = <T as fabric_system::Config>::AccountId {
...
decl_module! {
	...
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		fn offchain_worker(block_number: T::BlockNumber) {
			...
			// Using the `fabric_support::debug` module you have access to the same API exposed by
			// the `log` crate.
			...
			// to the storage and other included nobles.
			//
			// We can easily import `fabric_system` and retrieve a block hash of the parent block.
...
impl<T: Config> Module<T> {
	...
	fn fetch_price_and_send_signed() -> Result<(), &'static str> {
		...
		let results = signer.send_signed_transaction(
			|_account| {
				// Received price is wrapped into a call to `submit_price` public function of this noble.
	...
	fn fetch_price_and_send_raw_unsigned(block_number: T::BlockNumber) -> Result<(), &'static str> {
		...
		// noble. This means that the transaction, when executed, will simply call that function
	...
	fn fetch_price() -> Result<u32, http::Error> {
		...
		let body_str = tetcore_std::str::from_utf8(&body).map_err(|_| {
