...
use fabric_support::{RuntimeDebug, debug};
...
impl<T: fabric_system::Config> LeafDataProvider for fabric_system::Module<T> {
	type LeafData = (
...
pub struct Compact<H, T> {
	...
	pub tuple: T,
	_hash: tetcore_std::marker::PhantomData<H>,
}

impl<H, T> tetcore_std::ops::Deref for Compact<H, T> {
	type Target = T;
