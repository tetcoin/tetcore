...
//!
//! The MMR noble constructs a MMR from leaf data obtained on every block from
...
//! `fabric_system::Hashing`) so something compatible with external chains can be used (like
...
pub use noble_mmr_primitives as primitives;
...
pub trait Config<I = DefaultInstance>: fabric_system::Config {
	...
	type Hash: traits::Member + traits::MaybeSerializeDeserialize + tetcore_std::fmt::Debug
		+ tetcore_std::hash::Hash + AsRef<[u8]> + AsMut<[u8]> + Copy + Default + codec::Codec
		+ codec::EncodeLike;
	...
	/// (see [fabric_system::Module::deposit_log]) to make it available for Light Clients.
...
impl<T: Config<I>, I: Instance> Module<T, I> {
	fn offchain_key(pos: u64) -> tetcore_std::prelude::Vec<u8> {
