...
use noble_mmr_primitives::{Proof, Compact};
...
fn new_block() -> u64 {
	let number = fabric_system::Module::<Test>::block_number() + 1;
	let hash = H256::repeat_byte(number as u8);
	LEAF_DATA.with(|r| r.borrow_mut().a = number);

	fabric_system::Module::<Test>::initialize(
		&number,
		&hash,
		&Default::default(),
...
type BlockNumber = <Test as fabric_system::Config>::BlockNumber;
