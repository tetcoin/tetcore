...
//! noble on an already running chain.
...
use fabric_support::{decl_error, decl_module, decl_storage, decl_event, ensure, dispatch::DispatchResult};
...
type NegativeImbalanceOf<T> = <<T as Config>::Currency as Currency<<T as fabric_system::Config>::AccountId>>::NegativeImbalance;
...
pub struct EnsureFounder<T>(tetcore_std::marker::PhantomData<T>);
