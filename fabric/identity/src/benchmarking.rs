...
//! Identity noble benchmarking.

#![cfg(feature = "runtime-benchmarks")]

use super::*;

use fabric_system::{EventRecord, RawOrigin};
...
// Adds `r` registrars to the Identity Noble. These registrars will have set fees and fields.
...
benchmarks! {
	...
	set_identity {
		...
		let caller = {
			...
			let caller_origin: <T as fabric_system::Config>::Origin = RawOrigin::Signed(caller.clone()).into();
	...
	clear_identity {
		let caller: T::AccountId = whitelisted_caller();
		let caller_origin = <T as fabric_system::Config>::Origin::from(RawOrigin::Signed(caller.clone()));
		...
		let x in 1 .. T::MaxAdditionalFields::get() => {
			...
			let caller: T::AccountId = whitelisted_caller();
			let caller_origin = <T as fabric_system::Config>::Origin::from(RawOrigin::Signed(caller));
	...
	provide_judgement {
		...
		let user_origin = <T as fabric_system::Config>::Origin::from(RawOrigin::Signed(user.clone()));
	...
	kill_identity {
		...
		let target_origin: <T as fabric_system::Config>::Origin = RawOrigin::Signed(target.clone()).into();
