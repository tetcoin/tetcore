	...
	-> Result<(Vec<T::AccountId>, Vec<u8>), &'static str>
{
	...
	let call: <T as Config>::Call = fabric_system::Call::<T>::remark(vec![0; z as usize]).into();
...
benchmarks! {
	...
	as_multi_approve {
		...
		// Whitelist caller account from further DB operations.
		let caller_key = fabric_system::Account::<T>::hashed_key_for(&caller2);
		fabric_benchmarking::benchmarking::add_to_whitelist(caller_key.into());
