...
macro_rules! decl_tests {
	($test:ty, $ext_builder:ty, $existential_deposit:expr) => {
		...
		use crate::*;
		use tp_runtime::{FixedPointNumber, traits::{SignedExtension, BadOrigin}};
		use fabric_support::{
		...
		use noble_transaction_payment::{ChargeTransactionPayment, Multiplier};
		use fabric_system::RawOrigin;

		const ID_1: LockIdentifier = *b"1       ";
		const ID_2: LockIdentifier = *b"2       ";

		pub const CALL: &<$test as fabric_system::Config>::Call =
			&Call::Balances(noble_balances::Call::transfer(0, 0));
		...
		fn account_should_be_reaped() {
			<$ext_builder>::default().existential_deposit(1).monied(true).build().execute_with(|| {
				...
				assert!(!fabric_system::Account::<Test>::contains_key(&1));
			});
		}
		...
		fn lock_reasons_should_work() {
			<$ext_builder>::default()
				...
				.build()
				.execute_with(|| {
					noble_transaction_payment::NextFeeMultiplier::put(Multiplier::saturating_from_integer(1));
		...
		fn repatriating_reserved_balance_should_work() {
			<$ext_builder>::default().build().execute_with(|| {
				...
				assert_eq!(
					last_event(),
					Event::noble_balances(RawEvent::ReserveRepatriated(1, 2, 41, Status::Free)),
				);
				assert_eq!(Balances::reserved_balance(1), 69);
				assert_eq!(Balances::free_balance(1), 0);
				assert_eq!(Balances::reserved_balance(2), 0);
		...
		fn cannot_set_genesis_value_below_ed() {
			...
			let _ = noble_balances::GenesisConfig::<$test> {
				balances: vec![(1, 10)],
		...
		fn emit_events_with_reserve_and_unreserve() {
			<$ext_builder>::default()
				...
				.execute_with(|| {
					...
					assert_eq!(
						last_event(),
						Event::noble_balances(RawEvent::Reserved(1, 10)),
					);
					...
					assert_eq!(
						last_event(),
						Event::noble_balances(RawEvent::Unreserved(1, 5)),
					);
			...
			<$ext_builder>::default()
				...
				.execute_with(|| {
					...
					assert_eq!(
						events(),
						[
							Event::fabric_system(system::Event::NewAccount(1)),
					...
					assert_eq!(
						events(),
						[
							Event::noble_balances(RawEvent::DustLost(1, 99)),
							Event::fabric_system(system::Event::KilledAccount(1))
						]
					);
				});
		}
		...
		fn operations_on_dead_account_should_not_change_state() {
			...
			<$ext_builder>::default()
				...
				.build()
				.execute_with(|| {
					assert!(!fabric_system::Account::<Test>::contains_key(&1337));
