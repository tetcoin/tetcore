...
//! [FABRIC System module](../fabric_system/index.html) to perform these cross-cutting functions.
//!
...
//! # use fabric_executive as executive;
//! # pub struct UncheckedExtrinsic {};
//! # pub struct Header {};
//! # type Context = fabric_system::ChainContext<Runtime>;
...
///
/// Generic parameters:
/// - `System`: Something that implements `fabric_system::Config`
	...
	UnsignedValidator: ValidateUnsigned<Call=CallOf<Block::Extrinsic, Context>>,
{
	...
	fn initialize_block_impl(
		...
		if Self::runtime_upgraded() {
			...
			weight = weight.saturating_add(<fabric_system::Module::<System> as OnRuntimeUpgrade>::on_runtime_upgrade());
		...
		<fabric_system::Module<System>>::initialize(
			block_number,
			parent_hash,
			digest,
		...
		weight = weight.saturating_add(
			<fabric_system::Module<System> as OnInitialize<System::BlockNumber>>::on_initialize(*block_number)
		);
		weight = weight.saturating_add(
			<AllModules as OnInitialize<System::BlockNumber>>::on_initialize(*block_number)
		);
		weight = weight.saturating_add(
			<System::BlockWeights as fabric_support::traits::Get<_>>::get().base_block
		);
		<fabric_system::Module::<System>>::register_extra_weight_unchecked(weight, DispatchClass::Mandatory);

		fabric_system::Module::<System>::note_finished_initialize();
	}

	/// Returns if the runtime was upgraded since the last time this function was called.
	fn runtime_upgraded() -> bool {
		let last = fabric_system::LastRuntimeUpgrade::<System>::get();
		let current = <System::Version as fabric_support::traits::Get<_>>::get();

		if last.map(|v| v.was_upgraded(&current)).unwrap_or(true) {
			fabric_system::LastRuntimeUpgrade::<System>::put(
				fabric_system::LastRuntimeUpgradeInfo::from(current),
			);
			true
		} else {
			false
	...
	fn initial_checks(block: &Block) {
		...
		assert!(
			...
			&& <fabric_system::Module<System>>::block_hash(n - System::BlockNumber::one()) == *header.parent_hash(),
	...
	fn execute_extrinsics_with_book_keeping(
		...
		<fabric_system::Module<System>>::note_finished_extrinsics();
		<fabric_system::Module<System> as OnFinalize<System::BlockNumber>>::on_finalize(block_number);
	...
	pub fn finalize_block() -> System::Header {
		...
		<fabric_system::Module<System>>::note_finished_extrinsics();
		let block_number = <fabric_system::Module<System>>::block_number();
		<fabric_system::Module<System> as OnFinalize<System::BlockNumber>>::on_finalize(block_number);
		<AllModules as OnFinalize<System::BlockNumber>>::on_finalize(block_number);

		<fabric_system::Module<System>>::finalize()
	}
	...
	fn apply_extrinsic_with_len(
		...
		<fabric_system::Module<System>>::note_extrinsic(to_note);
		...
		<fabric_system::Module<System>>::note_applied_extrinsic(&r, dispatch_info);
	...
	fn final_checks(header: &System::Header) {
		...
		// remove temporaries
		let new_header = <fabric_system::Module<System>>::finalize();
	...
	pub fn offchain_worker(header: &System::Header) {
		...
		<fabric_system::Module<System>>::initialize(
			...
			header.parent_hash(),
			&digests,
			fabric_system::InitKind::Inspection,
		);

		// Fabric system only inserts the parent hash into the block hashes as normally we don't know
		// the hash for the header before. However, here we are aware of the hash and we can add it
		// as well.
		fabric_system::BlockHash::<System>::insert(header.number(), header.hash());
...
mod tests {
	...
	use noble_balances::Call as BalancesCall;
	use hex_literal::hex;
		...
		fabric_support::decl_module! {
			pub struct Module<T: Config> for enum Call where origin: T::Origin {
				...
				fn some_function(origin) {
					...
					let _ = fabric_system::ensure_signed(origin);
				}
				#[weight = (200, DispatchClass::Operational)]
				fn some_root_operation(origin) {
					let _ = fabric_system::ensure_root(origin);
				}
				#[weight = 0]
				fn some_unsigned_message(origin) {
					let _ = fabric_system::ensure_none(origin);
				}
	...
	parameter_types! {
		...
		pub BlockWeights: fabric_system::limits::BlockWeights =
			fabric_system::limits::BlockWeights::builder()
				.base_block(10)
	...
	impl fabric_support::traits::Get<tp_version::RuntimeVersion> for RuntimeVersion {
	...
	fn extra(nonce: u64, fee: Balance) -> SignedExtra {
		(
			fabric_system::CheckEra::from(Era::Immortal),
			fabric_system::CheckNonce::from(nonce),
			fabric_system::CheckWeight::new(),
			noble_transaction_payment::ChargeTransactionPayment::from(fee)
		)
	}
	...
	fn balance_transfer_dispatch_works() {
		let mut t = fabric_system::GenesisConfig::default().build_storage::<Runtime>().unwrap();
		noble_balances::GenesisConfig::<Runtime> {
			balances: vec![(1, 211)],
		}.assimilate_storage(&mut t).unwrap();
		let xt = TestXt::new(Call::Balances(BalancesCall::transfer(2, 69)), sign_extra(1, 0, 0));
		let weight = xt.get_dispatch_info().weight +
			<Runtime as fabric_system::Config>::BlockWeights::get().get(DispatchClass::Normal).base_extrinsic;
		let fee: Balance
			= <Runtime as noble_transaction_payment::Config>::WeightToFee::calc(&weight);
		...
		t.execute_with(|| {
			...
			assert!(r.is_ok());
			assert_eq!(<noble_balances::Module<Runtime>>::total_balance(&1), 142 - fee);
			assert_eq!(<noble_balances::Module<Runtime>>::total_balance(&2), 69);
		});
	}
	...
	fn bad_extrinsic_not_inserted() {
		...
		t.execute_with(|| {
			...
			assert_eq!(<fabric_system::Module<Runtime>>::extrinsic_index(), Some(0));
		});
	}
	...
	fn block_weight_limit_enforced() {
		...
		let block_weights = <Runtime as fabric_system::Config>::BlockWeights::get();
		...
		t.execute_with(|| {
			...
			assert_eq!(<fabric_system::Module<Runtime>>::block_weight().total(), base_block_weight);
			...
			for nonce in 0..=num_to_exhaust_block {
				...
				if nonce != num_to_exhaust_block {
					assert!(res.is_ok());
					assert_eq!(
						<fabric_system::Module<Runtime>>::block_weight().total(),
						//--------------------- on_initialize + block_execution + extrinsic_base weight
						(encoded_len + 5) * (nonce + 1) + base_block_weight,
					);
					assert_eq!(<fabric_system::Module<Runtime>>::extrinsic_index(), Some(nonce as u32 + 1));
				} else {
	...
	fn block_weight_and_size_is_stored_per_tx() {
		...
		t.execute_with(|| {
			...
			let base_block_weight = 175 + <Runtime as fabric_system::Config>::BlockWeights::get().base_block;
			...
			assert_eq!(<fabric_system::Module<Runtime>>::block_weight().total(), base_block_weight);
			assert_eq!(<fabric_system::Module<Runtime>>::all_extrinsics_len(), 0);
			...
			let extrinsic_weight = len as Weight + <Runtime as fabric_system::Config>::BlockWeights
				::get().get(DispatchClass::Normal).base_extrinsic;
			assert_eq!(
				<fabric_system::Module<Runtime>>::block_weight().total(),
				base_block_weight + 3 * extrinsic_weight,
			);
			assert_eq!(<fabric_system::Module<Runtime>>::all_extrinsics_len(), 3 * len);

			let _ = <fabric_system::Module<Runtime>>::finalize();
	...
	fn can_pay_for_tx_fee_on_full_lock() {
		...
		let execute_with_lock = |lock: WithdrawReasons| {
			let mut t = new_test_ext(1);
			t.execute_with(|| {
				<noble_balances::Module<Runtime> as LockableCurrency<Balance>>::set_lock(
					id,
					&1,
					110,
					lock,
				...
				let weight = xt.get_dispatch_info().weight + <Runtime as fabric_system::Config>
					::BlockWeights
					::get()
				...
				if lock == WithdrawReasons::except(WithdrawReasons::TRANSACTION_PAYMENT) {
					...
					assert_eq!(<noble_balances::Module<Runtime>>::total_balance(&1), 111 - fee);
				} else {
					assert_eq!(
						Executive::apply_extrinsic(xt),
						Err(InvalidTransaction::Payment.into()),
					);
					assert_eq!(<noble_balances::Module<Runtime>>::total_balance(&1), 111);
				}
			});
		};
	...
	fn block_hooks_weight_is_stored() {
		new_test_ext(1).execute_with(|| {
			...
			assert_eq!(<fabric_system::Module<Runtime>>::block_weight().total(), 175 + 10);
		})
	}
	...
	fn runtime_upgraded_should_work() {
		new_test_ext(1).execute_with(|| {
			...
			assert!(fabric_system::LastRuntimeUpgrade::<Runtime>::exists());
			...
			assert_eq!(
				...
				fabric_system::LastRuntimeUpgrade::<Runtime>::get(),
			);
			...
			fabric_system::LastRuntimeUpgrade::<Runtime>::take();
			assert!(Executive::runtime_upgraded());
			assert_eq!(
				Some(LastRuntimeUpgradeInfo { spec_version: 1.into(), spec_name: "test".into() }),
				fabric_system::LastRuntimeUpgrade::<Runtime>::get(),
	...
	fn all_weights_are_recorded_correctly() {
		new_test_ext(1).execute_with(|| {
			...
			let fabric_system_upgrade_weight = fabric_system::Module::<Runtime>::on_runtime_upgrade();
			let custom_runtime_upgrade_weight = CustomOnRuntimeUpgrade::on_runtime_upgrade();
			let runtime_upgrade_weight = <AllModules as OnRuntimeUpgrade>::on_runtime_upgrade();
			let fabric_system_on_initialize_weight = fabric_system::Module::<Runtime>::on_initialize(block_number);
			let on_initialize_weight = <AllModules as OnInitialize<u64>>::on_initialize(block_number);
			let base_block_weight = <Runtime as fabric_system::Config>::BlockWeights::get().base_block;

			// Weights are recorded correctly
			assert_eq!(
				fabric_system::Module::<Runtime>::block_weight().total(),
				fabric_system_upgrade_weight +
				custom_runtime_upgrade_weight +
				runtime_upgrade_weight +
				fabric_system_on_initialize_weight +
