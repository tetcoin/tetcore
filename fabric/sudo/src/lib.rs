...
//! ## Related Modules
//!
//! * [Democracy](../noble_democracy/index.html)
//!
//! [`Call`]: ./enum.Call.html
...
decl_module! {
	...
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		fn sudo(origin, call: Box<<T as Config>::Call>) -> DispatchResultWithPostInfo {
			...
			let res = call.dispatch_bypass_filter(fabric_system::RawOrigin::Root.into());
		...
		fn sudo_as(origin,
			...
			let res = call.dispatch_bypass_filter(fabric_system::RawOrigin::Signed(who).into());
