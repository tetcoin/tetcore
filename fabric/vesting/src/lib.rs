...
use fabric_support::{decl_module, decl_event, decl_storage, decl_error, ensure};
...
type MaxLocksOf<T> = <<T as Config>::Currency as LockableCurrency<<T as fabric_system::Config>::AccountId>>::MaxLocks;
...
decl_event!(
	pub enum Event<T> where AccountId = <T as fabric_system::Config>::AccountId, Balance = BalanceOf<T> {
...
mod tests {
	...
	impl ExtBuilder {
		...
		pub fn build(self) -> tet_io::TestExternalities {
			...
			noble_vesting::GenesisConfig::<Test> {
				vesting: vec![
