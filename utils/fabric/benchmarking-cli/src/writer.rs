...
use fabric_benchmarking::{BenchmarkBatch, BenchmarkSelector, Analysis, RegressionModel};
...
// This function takes a list of `BenchmarkBatch` and organizes them by noble into a `HashMap`.
...
fn map_results(batches: &[BenchmarkBatch]) -> Result<HashMap<(String, String), Vec<BenchmarkData>>, std::io::Error> {
	...
	let mut noble_benchmarks = Vec::new();
	...
	while let Some(batch) = batches_iter.next() {
		...
		let noble_string = String::from_utf8(batch.noble.clone()).unwrap();
		let instance_string = String::from_utf8(batch.instance.clone()).unwrap();
		let benchmark_data = get_benchmark_data(batch);
		noble_benchmarks.push(benchmark_data);

		// Check if this is the end of the iterator
		if let Some(next) = batches_iter.peek() {
			// Next noble is different than current noble, save and create new data.
			let next_noble = String::from_utf8(next.noble.clone()).unwrap();
			let next_instance = String::from_utf8(next.instance.clone()).unwrap();
			if next_noble != noble_string || next_instance != instance_string {
				all_benchmarks.insert((noble_string, instance_string), noble_benchmarks.clone());
				noble_benchmarks = Vec::new();
			}
		} else {
...
pub fn write_results(
	...
	// Organize results by noble into a JSON map
	let all_results = map_results(batches)?;
	for ((noble, instance), results) in all_results.iter() {
		let mut file_path = path.clone();
		// If a user only specified a directory...
		if file_path.is_dir() {
			// Check if there might be multiple instances benchmarked.
			if all_results.keys().any(|(p, i)| p == noble && i != instance) {
				// Create new file: "path/to/noble_name_instance_name.rs".
				file_path.push(noble.clone() + "_" + &instance.to_snake_case());
			} else {
				// Create new file: "path/to/noble_name.rs".
				file_path.push(noble.clone());
			}
...
mod test {
	use super::*;
	use fabric_benchmarking::{BenchmarkBatch, BenchmarkParameter, BenchmarkResults};

	fn test_data(noble: &[u8], benchmark: &[u8], param: BenchmarkParameter, base: u32, slope: u32) -> BenchmarkBatch {
		...
		return BenchmarkBatch {
			noble: [noble.to_vec(), b"_noble".to_vec()].concat(),
	...
	fn map_results_works() {
		...
		let first_benchmark = &mapped_results.get(
			&("first_noble".to_string(), "instance".to_string())
		).unwrap()[0];
		...
		let second_noble_benchmark = &mapped_results.get(
			&("second_noble".to_string(), "instance".to_string())
		).unwrap()[0];
		assert_eq!(second_noble_benchmark.name, "first_benchmark");
		check_data(second_noble_benchmark, "c", 3, 4);
	}
}
