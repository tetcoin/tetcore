...
use fabric_benchmarking::{Analysis, BenchmarkBatch, BenchmarkSelector};
		...
		match results {
			Ok(batches) => {
				...
				for batch in batches.into_iter() {
					...
					println!(
						"Noble: {:?}, Extrinsic: {:?}, Lowest values: {:?}, Highest values: {:?}, Steps: {:?}, Repeat: {:?}",
						String::from_utf8(batch.noble).expect("Encoded from String; qed"),
