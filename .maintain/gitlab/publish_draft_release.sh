...
last_version=$(last_github_release 'tetcoin/tetcore')

release_text="$(./generate_release_text.sh "$last_version" "$version")"

echo "[+] Pushing release to github"
# Create release on github
release_name="Tetcore $version"
...
out=$(curl -s -X POST --data "$data" -H "Authorization: token $GITHUB_RELEASE_TOKEN" "$api_base/tetcoin/tetcore/releases")
...
msg_body=$(cat <<EOF
**Release pipeline for Tetcore $version complete.**
Draft release created: $html_url
EOF
)
formatted_msg_body=$(cat <<EOF
<strong>Release pipeline for Tetcore $version complete.</strong><br />
Draft release created: $html_url
EOF
)
