...
pub trait SimpleBitOps:
	Sized + Clear +
	tetcore_std::ops::BitOr<Self, Output = Self> +
	tetcore_std::ops::BitXor<Self, Output = Self> +
	tetcore_std::ops::BitAnd<Self, Output = Self>
{}
impl<T:
	Sized + Clear +
	tetcore_std::ops::BitOr<Self, Output = Self> +
...
pub trait Hash: 'static + MaybeSerializeDeserialize + Debug + Clone + Eq + PartialEq + Hasher<Out = <Self as Hash>::Output> {
	...
	type Output: Member + MaybeSerializeDeserialize + Debug + tetcore_std::hash::Hash
...
sp_core::impl_maybe_marker!(
	...
	trait MaybeHash: tetcore_std::hash::Hash;
...
{
	/// Header number.
	type Number: Member + MaybeSerializeDeserialize + Debug + tetcore_std::hash::Hash + Copy +
		MaybeDisplay + AtLeast32BitUnsigned + Codec + tetcore_std::str::FromStr + MaybeMallocSizeOf;
	/// Header hash type
	type Hash: Member + MaybeSerializeDeserialize + Debug + tetcore_std::hash::Hash + Ord
...
pub trait SignedExtension: Codec + Debug + Sync + Send + Clone + Eq + PartialEq {
	...
	fn identifier() -> Vec<&'static str> {
		tetcore_std::vec![Self::IDENTIFIER]
	}
}
...
macro_rules! impl_opaque_keys {
	(
		...
		impl $name {
			...
			pub fn generate(seed: Option<$crate::tetcore_std::vec::Vec<u8>>) -> $crate::tetcore_std::vec::Vec<u8> {
				let keys = Self{
					$(
						$field: <
							<
			...
			pub fn into_raw_public_keys(
				self,
			) -> $crate::tetcore_std::vec::Vec<($crate::tetcore_std::vec::Vec<u8>, $crate::KeyTypeId)> {
			...
			pub fn decode_into_raw_public_keys(
				encoded: &[u8],
			) -> Option<$crate::tetcore_std::vec::Vec<($crate::tetcore_std::vec::Vec<u8>, $crate::KeyTypeId)>> {
