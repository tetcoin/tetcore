...
/// A set of runtime APIs usable by rich offchain calls.
///
/// Most of the methods in `Externalities` should be limited only to
/// offchain workers. However there are cases, when we want to
/// allow Runtime API calls from the node to access some of them.
///
/// This trait collects all externalities that can be safely accessed
/// by offchain calls, that are not necessarily offchain workers.
pub trait CallExternalities: Send {
...
impl<T: CallExternalities + ?Sized> CallExternalities for Box<T> {
	fn timestamp(&mut self) -> Timestamp {
		(&mut **self).timestamp()
	}
...
impl<T: CallExternalities> CallExternalities for LimitedExternalities<T> {
	fn timestamp(&mut self) -> Timestamp {
		self.check(Capability::Timestamp, "timestamp");
		self.externalities.timestamp()
	}

	fn sleep_until(&mut self, deadline: Timestamp) {
		self.check(Capability::Timestamp, "sleep_until");
...
#[cfg(feature = "std")]
sp_externalities::decl_extension! {
	/// The offchain call extensions available for rich offchain calls.
	pub struct OffchainCallExt(Box<dyn CallExternalities>);
}

#[cfg(feature = "std")]
impl OffchainCallExt {
	/// Create a new instance of `Self`.
	pub fn new<O: CallExternalities + 'static>(offchain: O) -> Self {
		Self(Box::new(offchain))
	}
}
