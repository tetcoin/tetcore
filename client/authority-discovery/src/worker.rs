...
use codec::Decode;
use tet_libp2p::{core::multiaddr, multihash::{Multihash, Hasher}};
...
pub struct Worker<Client, Network, Block, DhtEventStream> {
	...
	in_flight_lookups: HashMap<tet_libp2p::kad::record::Key, AuthorityId>,
	...
	DhtEventStream: Stream<Item = DhtEvent> + Unpin,
{
	...
	fn handle_dht_value_found_event(
		&mut self,
		values: Vec<(tet_libp2p::kad::record::Key, Vec<u8>)>,
	) -> Result<()> {
...
pub trait NetworkProvider: NetworkStateInfo {
	...
	fn put_value(&self, key: tet_libp2p::kad::record::Key, value: Vec<u8>);

	/// Start getting a value from the Dht.
	fn get_value(&self, key: &tet_libp2p::kad::record::Key);
}
...
fn hash_authority_id(id: &[u8]) -> tet_libp2p::kad::record::Key {
	tet_libp2p::kad::record::Key::new(&tet_libp2p::multihash::Sha2_256::digest(id))
}
