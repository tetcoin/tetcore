...
pub type PeersLightClient = Client<
	tetcore_test_runtime_client::LightBackend,
	tetcore_test_runtime_client::LightExecutor,
	Block,
	tetcore_test_runtime_client::runtime::RuntimeApi
>;
...
pub enum PeersClient {
	Full(Arc<PeersFullClient>, Arc<tetcore_test_runtime_client::Backend>),
	Light(Arc<PeersLightClient>, Arc<tetcore_test_runtime_client::LightBackend>),
}
...
pub struct Peer<D> {
	...
	select_chain: Option<LongestChain<tetcore_test_runtime_client::Backend, Block>>,
	backend: Option<Arc<tetcore_test_runtime_client::Backend>>,
...
impl<D> Peer<D> {
	...
	pub fn select_chain(&self) -> Option<LongestChain<tetcore_test_runtime_client::Backend, Block>> {
	...
	pub fn generate_blocks<F>(&mut self, count: usize, origin: BlockOrigin, edit_block: F) -> H256
		where F: FnMut(BlockBuilder<Block, PeersFullClient, tetcore_test_runtime_client::Backend>) -> Block
	{
	...
	fn generate_blocks_at<F>(
		...
		announce_block: bool,
	) -> H256 where F: FnMut(BlockBuilder<Block, PeersFullClient, tetcore_test_runtime_client::Backend>) -> Block {
...
pub enum BlockImportAdapter<Transaction> {
	Full(
		Arc<Mutex<dyn BlockImport<
			Block,
			Transaction = TransactionFor<tetcore_test_runtime_client::Backend, Block>,
	...
	Light(
		Arc<Mutex<dyn BlockImport<
			Block,
			Transaction = TransactionFor<tetcore_test_runtime_client::LightBackend, Block>,
			Error = ConsensusError
		> + Send>>,
		PhantomData<Transaction>,
	),
...
pub trait TestNetFactory: Sized {
	...
	fn add_light_peer(&mut self) {
		let (c, backend) = tetcore_test_runtime_client::new_light();
		let client = Arc::new(c);
		let (
			block_import,
			justification_import,
