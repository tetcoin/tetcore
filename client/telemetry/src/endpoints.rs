...
impl TelemetryEndpoints {
	...
	pub fn new(endpoints: Vec<(String, u8)>) -> Result<Self, tetsy_libp2p::multiaddr::Error> {
		let endpoints: Result<Vec<(Multiaddr, u8)>, tetsy_libp2p::multiaddr::Error> = endpoints
			.iter()
...
fn url_to_multiaddr(url: &str) -> Result<Multiaddr, tetsy_libp2p::multiaddr::Error> {
	...
	if let Ok(ma) = tetsy_libp2p::multiaddr::from_url(url) {
		return Ok(ma);
	}
