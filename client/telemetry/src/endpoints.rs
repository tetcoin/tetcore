...
impl TelemetryEndpoints {
	...
	pub fn new(endpoints: Vec<(String, u8)>) -> Result<Self, tet_libp2p::multiaddr::Error> {
		let endpoints: Result<Vec<(Multiaddr, u8)>, tet_libp2p::multiaddr::Error> = endpoints
			.iter()
...
fn url_to_multiaddr(url: &str) -> Result<Multiaddr, tet_libp2p::multiaddr::Error> {
	...
	if let Ok(ma) = tet_libp2p::multiaddr::from_url(url) {
		return Ok(ma);
	}
