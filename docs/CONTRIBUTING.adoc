...
. PRs that break the external API must be tagged with https://github.com/tetcoin/tetcore/labels/D2-breaksapi[`D2-breaksapi`], when it changes the FRAME or consensus of running system with https://github.com/tetcoin/tetcore/labels/B3-breaksconsensus[`B3-breaksconsensus`].
...
. Submit all this as a PR against the Polkadot Repo. Link to your Polkadot PR in the _description_ of your Tetcore PR as "polkadot companion: [URL]"
. Now you should see that the `check_polkadot` CI job will build your Tetcore PR agains the mentioned Polkadot branch in your PR description.
...
. Once the Tetcore pull request runs green, a member of the `tetsy` github group can comment on the Tetcore pull request with `bot merge` which will:
    ...
    - In case the pull request origins from https://github.com/tetcoin/polkadot directly and not from a fork:
...
If your PR is reviewed well, but a Polkadot PR is missing, signal it with https://github.com/tetcoin/tetcore/labels/A7-needspolkadotpr[`A7-needspolkadotpr`] to prevent it from getting automatically merged.
