...
The default (testing) application pulling together our recommended setup of tetcore-client with a wasm-contracts-supporting frame-runtime.  The node pulls it all together, constructs the (upgradable) runtime, and wires up the client around it. You can find an example client, which includes a full wasm-contracts chain in  `node`. This is also what is being built and run if you do `cargo run`.
...
- **tetkey**
  Subkey is a client library to generate keys and sign transactions to send to a tetcore node.
- **chain-spec-builder**
